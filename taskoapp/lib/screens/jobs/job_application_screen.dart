...
class _JobApplicationScreenState extends State<JobApplicationScreen> {
    ...
    try {
      ...
      if (mounted) {
        ...
        if (profile != null) {
          ...
          // Load CVs
          if (profile['cvUrl'] != null && profile['cvUrl'].toString().isNotEmpty) {
            final id = 'cv_profile';
            _cvAttachments.add({
              'id': id,
          ...
          // Load Cover Letters
          if (profile['coverLetterUrl'] != null && profile['coverLetterUrl'].toString().isNotEmpty) {
            final id = 'cl_profile';
          ...
          if (profile['experience'] != null && profile['experience'] is List) {
            ...
            for (var exp in profile['experience']) {
              if (exp['certificateUrl'] != null && exp['certificateUrl'].toString().isNotEmpty) {
                _otherAttachments.add({
                  'id': 'exp_$i',
                  'name': exp['fileName'] ?? 'Zeugnis - ${exp['company'] ?? 'Unbekannt'}',
          ...
          if (profile['education'] != null && profile['education'] is List) {
            ...
            for (var edu in profile['education']) {
              if (edu['certificateUrl'] != null && edu['certificateUrl'].toString().isNotEmpty) {
                _otherAttachments.add({
                  'id': 'edu_$i',
                  'name': edu['fileName'] ?? 'Zeugnis - ${edu['institution'] ?? 'Unbekannt'}',
          ...
          if (profile['qualifications'] != null && profile['qualifications'] is List) {
            int i = 0;
            for (var qual in profile['qualifications']) {
              if (qual['certificateUrl'] != null && qual['certificateUrl'].toString().isNotEmpty) {
                _otherAttachments.add({
                  'id': 'qual_$i',
                  'name': qual['fileName'] ?? 'Zertifikat - ${qual['name'] ?? 'Unbekannt'}',
  ...
  Future<void> _submitApplication() async {
    ...
    if (!hasCv) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text('Bitte fügen Sie mindestens einen Lebenslauf bei.')),
      );
      return;
    }
    ...
    return Card(
      ...
      child: Theme(
        ...
        child: ExpansionTile(
              ...
              TextButton.icon(
                onPressed: () async {
                  final result = await Navigator.push(
                    context,
                    MaterialPageRoute(
                      builder: (context) => CandidateProfileEditScreen(initialProfile: _profile!),
                    ),
                  );
                  if (result == true) {
                    _loadProfile();
          ...
          children: [
            if (experience.isEmpty && education.isEmpty && languages.isEmpty && skillsList.isEmpty)
              const Padding(
            ...
            if (experience.isNotEmpty) ...[
              ...
              ...experience.map((exp) {
                ...
                final end = exp['endDate'] != null && exp['endDate'].toString().isNotEmpty 
                  ...
                  child: Column(
                    ...
                    children: [
                      ...
                      if (exp['fileName'] != null) ...[
                        ...
                        Row(
                          children: [
                            const Icon(Icons.attachment, size: 16, color: Colors.teal),
            ...
            if (education.isNotEmpty) ...[
              ...
              ...education.map((edu) {
                ...
                final end = edu['endDate'] != null && edu['endDate'].toString().isNotEmpty 
            ...
            if (languages.isNotEmpty) ...[
              ...
              ...languages.map((lang) => Padding(
                ...
                child: Row(
                  mainAxisAlignment: MainAxisAlignment.spaceBetween,
                  children: [
                    Text(lang['language'] ?? '', style: const TextStyle(fontSize: 15)),
                    Text(
                      lang['level'] ?? '', 
                      style: TextStyle(color: Colors.grey.shade600, fontSize: 14),
                    ),
            ...
            if (attachments.isEmpty)
              ...
              ...attachments.map((att) {
                ...
                return Container(
                  ...
                  decoration: BoxDecoration(
                    color: isSelected ? const Color(0xFFF0FDFA) : Colors.white, // teal-50
                    border: Border.all(
                      color: isSelected ? const Color(0xFF99F6E4) : Colors.grey.shade200, // teal-200
                    ),
            ...
            InkWell(
              ...
              child: Container(
                ...
                child: Row(
                  ...
                  children: [
                    const Icon(Icons.upload_file, size: 16, color: Colors.black87),
                    const SizedBox(width: 8),
                    Text(
                      uploadLabel,
                      style: const TextStyle(
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      backgroundColor: const Color(0xFFF9FAFB), // Light gray background like web
      appBar: AppBar(
                    ...
                    child: Column(
                      ...
                      children: [
                        ...
                        _buildAttachmentCard(
                          title: 'Anschreiben',
                          subtitle: 'Nutze dein gespeichertes Anschreiben oder lade ein neues hoch.',
                          attachments: _coverLetterAttachments,
                          uploadLabel: 'Weiteres Anschreiben hochladen',
                          onUpload: () {
                            ScaffoldMessenger.of(context).showSnackBar(
                              const SnackBar(content: Text('Upload-Funktion in der App noch nicht verfügbar.')),
                            );
                          },
                        ),
                        ...
                        _buildAttachmentCard(
                          title: 'Lebenslauf',
                          subtitle: 'Nutze deinen gespeicherten Lebenslauf oder lade einen neuen hoch.',
                        ...
                        _buildAttachmentCard(
                          ...
                          subtitle: 'Nutze deine gespeicherten Dokumente (Zeugnisse, Zertifikate).',
