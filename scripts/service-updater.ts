...
const SERVICE_REPLACEMENTS: ServiceReplacement[] = [
  ...
  // DOC REFERENCES - CUSTOMERS
  {
    ...
    newPattern: (match, customerId) => `doc(db, 'companies', companyId, 'customers', ${customerId})`,
  ...
  // DOC REFERENCES - STOCK MOVEMENTS
  {
    ...
    newPattern: (match, movementId) => `doc(db, 'companies', companyId, 'stockMovements', ${movementId})`,
...
class ServiceUpdater {
  ...
  async findAllFirestoreUsages(): Promise<void> {
    ...
    const filesToScan = [
      ...fs.readdirSync('src/services').filter(f => f.endsWith('.ts')).map(f => `src/services/${f}`),
