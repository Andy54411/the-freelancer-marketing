...
async function validateMigration() {
  ...
  for (const collectionName of collections) {
    try {
      ...
      const newCollectionRef = db.collection('companies').doc(targetCompanyId).collection(collectionName);
      ...
      if (newSnapshot.size > 0) {
        console.log(`   📋 Migrierte Dokument-IDs: ${newSnapshot.docs.map(doc => doc.id).join(', ')}`);
      }
    ...
    try {
      ...
      if (snapshot.size > 0) {
        ...
        docsWithoutCompanyId.forEach(doc => {
          console.log(`      - ID: ${doc.id}, User: ${doc.data.userId}, Created: ${doc.data.createdAt}`);
        });
      }
      console.log('');
