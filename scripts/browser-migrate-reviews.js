...
async function migrateReviewsToSubcollections() {
  try {
    ...
    if (!db) {
      throw new Error(
        'Firebase db nicht verfügbar. Stelle sicher, dass du auf localhost:3000 bist.'
      );
    }

    // Firebase v9+ Funktionen importieren
    const { collection, getDocs, doc, setDoc, query, where, deleteDoc } =
    ...
    for (const reviewDoc of reviewsSnapshot.docs) {
      ...
      try {
        ...
        const subcollectionRef = doc(db, 'companies', reviewData.providerId, 'reviews', reviewId);
...
async function validateSpecificReview() {
  try {
    ...
    const subcollectionRef = doc(db, 'companies', specificProviderId, 'reviews', specificReviewId);
...
async function deleteOldReviews() {
  const confirmation = confirm(
    '⚠️ WARNUNG: Möchtest du wirklich alle alten Reviews aus der Top-Level Collection löschen?\n\nStelle sicher, dass die Migration erfolgreich war!'
  );
