...
// Lade die Umgebungsvariablen (falls .env.local existiert)
require('dotenv').config({ path: '../.env.local' });

// Firebase-Konfiguration aus den Umgebungsvariablen
...
async function createTestReviews() {
  try {
    console.log('Erstelle Test-Bewertungen für Mietkoch Andy...');

    // Provider-ID - Diese muss mit einer echten Provider-ID aus der Datenbank übereinstimmen
    // Sie können eine echte Provider-ID aus Ihrer Datenbank verwenden oder eine neue erstellen
    ...
    console.log('   node create-test-reviews.js <echte-provider-id>');
    ...
    for (let i = 0; i < reviews.length; i++) {
      ...
      console.log(
        `✅ Bewertung ${i + 1}/5 erstellt: ${reviews[i].reviewerName} (${reviews[i].rating}⭐) - ID: ${docRef.id}`
      );
    }
    ...
    if (error.code === 'permission-denied') {
      ...
      console.log('2. Stellen Sie sicher, dass die Firebase-Konfiguration korrekt ist');
      console.log('3. Überprüfen Sie die .env.local Datei mit den Firebase-Umgebungsvariablen');
    }

    if (error.code === 'project-not-found') {
      console.log('\n💡 Das Firebase-Projekt wurde nicht gefunden.');
      console.log('Überprüfen Sie die NEXT_PUBLIC_FIREBASE_PROJECT_ID in der .env.local Datei');
    }
  }
}
