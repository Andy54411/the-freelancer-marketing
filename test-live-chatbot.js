...
async function testChatbotResponses() {
  ...
  try {
    const instruction1 = await getSystemInstruction(
      mockFirestore,
      console.error,
      userMessage1,
      chatHistory1
    ...
    console.log(
      '📊 Auftragserkennung:',
      instruction1.includes('Auftrag #Bw0brkeOrQU7oGTJuTop') ? '✅ Erkannt' : '❌ Nicht erkannt'
    );
    console.log(
      '📊 Tasko-Kontext:',
      instruction1.includes('Tasko') ? '✅ Vorhanden' : '❌ Fehlend'
    );
    console.log(
      '📊 Stornierungsrichtlinien:',
      instruction1.includes('Stornierung') ? '✅ Vorhanden' : '❌ Fehlend'
    );
    console.log(
      '📊 Keine Plattform-Fragen:',
      !instruction1.includes('Welche Plattform') ? '✅ Korrekt' : '❌ Problematisch'
    );
  ...
  try {
    ...
    console.log(
      '📊 Keine Auftragsdaten:',
      !instruction2.includes('Auftrag #') ? '✅ Korrekt' : '❌ Unerwartete Auftragsdaten'
    );
    console.log(
      '📊 Tasko-Kontext:',
      instruction2.includes('Tasko') ? '✅ Vorhanden' : '❌ Fehlend'
    );
    console.log(
      '📊 Grundregeln:',
      instruction2.includes('ABSOLUTE GRUNDREGELN') ? '✅ Vorhanden' : '❌ Fehlend'
    );
  } catch (error) {
  ...
  try {
    ...
    console.log(
      ...
      instruction3.includes('Bw0brkeOrQU7oGTJuTop') ? '✅ Erkannt' : '❌ Nicht erkannt'
    );
    console.log(
      '📊 Zweite Auftragsnummer:',
      instruction3.includes('TestOrder123') ? '✅ Erkannt' : '❌ Nicht erkannt'
    );
  } catch (error) {
