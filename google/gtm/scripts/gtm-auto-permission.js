...
async function autoSetupGTMPermissions() {
  try {
    ...
    const accountsResponse = await makeHTTPSRequest('https://www.googleapis.com/tagmanager/v2/accounts', {
      method: 'GET',
      headers: {
      ...
      for (const account of accountsResponse.account) {
        ...
        try {
          ...
          if (containersResponse.container) {
            const targetContainer = containersResponse.container.find(c => c.publicId === 'GTM-TG3H7QHX');
            ...
            if (targetContainer) {
              ...
              envContent = envContent.replace(/GTM_ACCOUNT_ID=\d+/, `GTM_ACCOUNT_ID=${account.accountId}`);
              envContent = envContent.replace(/GTM_CONTAINER_ID=GTM-TG3H7QHX/, `GTM_CONTAINER_ID=${targetContainer.containerId}`);
              ...
              try {
                const testResult = execSync('node gtm-upload-service-account.js gtm-dsgvo-triggers-fixed.json --dry-run', { 
                  encoding: 'utf8',
                  stdio: 'pipe'
                });
                ...
                console.log('\n🎉 BEREIT FÜR UPLOAD!');
                console.log('=====================');
                console.log('Führen Sie aus: node gtm-upload-service-account.js gtm-dsgvo-triggers-fixed.json');
