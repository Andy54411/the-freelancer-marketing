  ...
  try {
    ...
    if (accountsResponse.account && accountsResponse.account.length > 0) {
      ...
      if (targetContainer && targetAccount) {
        ...
        await uploadDSGVOTriggers(accessToken.token, targetAccount.accountId, targetContainer.containerId);
  ...
  try {
    ...
    if (configData.variables) {
      ...
      for (const variable of configData.variables) {
        try {
          const result = await makeHTTPSRequest(`${baseUrl}/variables`, {
            method: 'POST',
            headers: {
    ...
    if (configData.triggers) {
      ...
      for (const trigger of configData.triggers) {
        try {
          const result = await makeHTTPSRequest(`${baseUrl}/triggers`, {
            method: 'POST',
            headers: {
              'Authorization': `Bearer ${accessToken}`,
              'Content-Type': 'application/json'
      ...
      res.on('end', () => {
        try {
          ...
          if (res.statusCode >= 200 && res.statusCode < 300) {
            ...
            reject(new Error(`API Error ${res.statusCode}: ${jsonData.error?.message || responseData}`));
          }
        } catch (parseError) {
