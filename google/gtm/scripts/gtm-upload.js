...
class GTMAPIClient {
  ...
  async publishWorkspace(versionName = 'DSGVO Trigger Upload') {
    ...
    const publishResponse = await this.makeRequest(`/versions/${response.containerVersionId}/publish`, 'POST');
  ...
  async uploadConfiguration(configPath) {
    try {
      ...
      console.log(`📊 Results: ${results.triggers.length} triggers, ${results.variables.length} variables, ${results.tags.length} tags`);
...
async function uploadToGTM() {
  try {
    ...
    console.log(`🔗 https://tagmanager.google.com/#/container/accounts/${GTM_CONFIG.accountId}/containers/${GTM_CONFIG.containerId}`);
