...
class GTMAPIClient {
  ...
  /**
   * Erstellt oder holt einen Default Workspace
   */
  async getDefaultWorkspace() {
    try {
      ...
      if (response.workspace && response.workspace.length > 0) {
        // Finde den Default Workspace
        const defaultWorkspace = response.workspace.find(w => w.name === 'Default Workspace');
        if (defaultWorkspace) {
          return defaultWorkspace.workspaceId;
        }
        // Oder nimm den ersten verfügbaren
   ...
   */
  async createVariable(variableData, workspaceId) {
    try {
      const response = await this.makeRequest(`workspaces/${workspaceId}/variables`, 'POST', variableData);
      return response;
    } catch (error) {
   ...
   */
  async createTrigger(triggerData, workspaceId) {
    try {
      const response = await this.makeRequest(`workspaces/${workspaceId}/triggers`, 'POST', triggerData);
      return response;
    } catch (error) {
  ...
  async uploadConfiguration(configPath, dryRun = false) {
    try {
      ...
      const workspaceId = await this.getDefaultWorkspace();
      console.log(`✅ Workspace gefunden: ${workspaceId}`);
...
async function main() {
  ...
  const configPath = args.find(arg => arg.endsWith('.json')) || 'gtm-dsgvo-triggers-fixed.json';
