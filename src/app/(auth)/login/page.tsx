// page.tsx (Ihre Registrierungsseite, z.B. src/app/(auth)/login/page.tsx)
"use client";
...
// NEU: LoginPopupProps explizit importieren
          ...
          <CardContent>
            <form onSubmit={handleRegister} className="space-y-4">
              <div className="grid gap-1.5">
                ...
                <Input type="email" id="emailReg" value={email} onChange={(e) => setEmail(e.target.value)} required className="px-3 py-2 border rounded-md focus:ring-2 focus:ring-[#14ad9f] w-full h-10 text-sm" />
              </div>
              <div className="grid gap-1.5">
                <Label htmlFor="passwordReg" className="text-[#14ad9f] font-medium">Passwort</Label>
                <Input type="password" id="passwordReg" value={password} onChange={(e) => setPassword(e.target.value)} required className="px-3 py-2 border rounded-md focus:ring-2 focus:ring-[#14ad9f] w-full h-10 text-sm" />
              </div>
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div className="grid gap-1.5">
                  <Label htmlFor="firstnameReg" className="text-[#14ad9f] font-medium">Vorname</Label>
                  <Input type="text" id="firstnameReg" value={firstname} onChange={(e) => setFirstname(e.target.value)} required className="px-3 py-2 border rounded-md focus:ring-2 focus:ring-[#14ad9f] w-full h-10 text-sm" />
                </div>
                <div className="grid gap-1.5">
                  <Label htmlFor="lastnameReg" className="text-[#14ad9f] font-medium">Nachname</Label>
                  <Input type="text" id="lastnameReg" value={lastname} onChange={(e) => setLastname(e.target.value)} required className="px-3 py-2 border rounded-md focus:ring-2 focus:ring-[#14ad9f] w-full h-10 text-sm" />
                </div>
              </div>
              <div className="grid gap-1.5">
                ...
                <div className="flex items-center gap-2">
                  <div className="relative">
                    <button type="button" className="px-3 py-2 border rounded-l-md focus:ring-2 focus:ring-[#14ad9f] flex items-center h-10 text-sm bg-gray-50 hover:bg-gray-100" onClick={() => setIsDropdownOpen(!isDropdownOpen)} aria-haspopup="true" aria-expanded={isDropdownOpen}>
                      <Flag code={europeanCountryCodes.find(country => country.dialCode === selectedCountryCode)?.flag} className="w-5 h-auto mr-1.5" />
                      {selectedCountryCode}
                    ...
                    {isDropdownOpen && (
                      <div className="absolute z-20 mt-1 w-52 bg-white shadow-lg rounded-md max-h-48 overflow-y-auto border border-gray-200">
                        {europeanCountryCodes.map((country) => (
                          <div key={country.dialCode} className="flex items-center p-2 cursor-pointer hover:bg-gray-100 text-sm" onClick={() => { setSelectedCountryCode(country.dialCode); setIsDropdownOpen(false); }}>
                  ...
                  <Input type="tel" id="phoneReg" value={phone} onChange={(e) => setPhone(e.target.value.replace(/\D/g, ''))} required placeholder="123456789" className="px-3 py-2 border rounded-r-md focus:ring-2 focus:ring-[#14ad9f] flex-1 h-10 text-sm" />
                </div>
              </div>
              <div className="grid gap-1.5">
                <Label htmlFor="postalCodeReg" className="text-[#14ad9f] font-medium">Postleitzahl</Label>
                <Input type="text" id="postalCodeReg" value={postalCode} onChange={(e) => setPostalCode(e.target.value)} required className="px-3 py-2 border rounded-md focus:ring-2 focus:ring-[#14ad9f] w-full h-10 text-sm" />
              </div>

              {error && <p className="text-red-500 text-xs text-center p-2 bg-red-50 rounded-md">{error}</p>}

              <Button type="submit" className="w-full bg-[#14ad9f] text-white hover:bg-teal-600 transition-colors duration-300 py-2.5 text-sm font-semibold" disabled={loading}>
                {loading ? 'Konto wird erstellt...' : 'Konto erstellen'}
              </Button>

              <p className="text-center text-xs text-gray-600">
                Du hast bereits ein Konto?{' '}
                <button type="button" onClick={() => { setError(null); setIsLoginPopupOpen(true); }} className="text-[#14ad9f] hover:underline font-medium">
                  Hier einloggen
                </button>
              </p>
            </form>
