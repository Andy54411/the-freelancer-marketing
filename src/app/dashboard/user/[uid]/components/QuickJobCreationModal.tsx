...
export function QuickJobCreationModal({
  isOpen,
  onClose,
  initialCategory,
  initialSubcategory,
}: QuickJobCreationModalProps) {
  ...
  useEffect(() => {
    if (!isOpen) {
      ...
      if (registration.selectedCategory || registration.description || registration.jobPostalCode) {
        // Füge hier weitere wichtige Felder hinzu
      ...
      if (initialCategory && initialSubcategory && currentStep === 'categorySelection') {
        ...
        if (
          registration.selectedCategory !== initialCategory ||
          registration.selectedSubcategory !== initialSubcategory
        ) {
  ...
  const getStepTitle = (step: JobCreationStep) => {
    switch (step) {
      case 'categorySelection':
        return '1. Service auswählen';
      case 'addressInput':
        return '2. Adresse eingeben';
      case 'dateTimeAndDescription':
        return '3. Datum & Details';
      case 'anbieterMatching':
        return '4. Anbieter finden';
      case 'confirmation':
        return '5. Auftrag prüfen';
      case 'payment':
        return '6. Zahlung abschließen';
      default:
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-50 p-4">
      <div className="bg-white dark:bg-gray-800 rounded-lg shadow-xl w-full max-w-2xl max-h-[90vh] overflow-hidden relative flex flex-col">
        ...
        <div className="p-6 flex-grow overflow-y-auto">
          {currentStep === 'categorySelection' && <CategorySelectionStep onNext={handleNextStep} />}
          ...
          {currentStep === 'payment' && (
            <div>
              ...
              <button onClick={onClose} className="mt-4 px-4 py-2 bg-green-500 text-white rounded">
                Zahlung erfolgreich!
              </button>
            </div>
          )}
