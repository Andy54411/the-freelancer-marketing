...
const ProfilePictureUploadModal: React.FC<ProfilePictureUploadModalProps> = ({ currentUser, onClose, onSuccess }) => {
        ...
        <Modal title="Profilbild hochladen" onClose={onClose}>
            <div className="space-y-4">
                ...
                >
                    ...
                    <p className="mt-2 text-sm text-gray-600">{isDragActive ? 'Bild hier ablegen...' : 'Bild hierher ziehen oder klicken, um es auszuwählen.'}</p>
                </div>

                {preview && (
                    <div className="text-center"><p className="text-sm font-medium text-gray-700 mb-2">Vorschau:</p><Image src={preview} alt="Profilbild Vorschau" width={128} height={128} className="rounded-full object-cover mx-auto border-4 border-white shadow-md" /></div>
                )}

                <div className="flex justify-end gap-3 pt-4"><Button variant="outline" onClick={onClose} disabled={uploading}>Abbrechen</Button><Button onClick={handleUpload} disabled={!file || uploading}>{uploading ? <><FiLoader className="animate-spin mr-2" />Wird hochgeladen...</> : 'Hochladen & Speichern'}</Button></div>
            </div>
        </Modal>
    );
};
