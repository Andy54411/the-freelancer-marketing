...
// KORREKTUR: Import aus der ZENTRALEN Typ-Datei '@/types/types'
// Dies behebt den Fehler "BookingCharacteristics" nicht gefunden.
import { BookingCharacteristics, DateTimeSelectionPopupProps } from '@/types/types';
// Annahme: Der Pfad zu dieser Hilfsfunktion ist korrekt
import { getBookingCharacteristics } from '@/app/auftrag/get-started/[unterkategorie]/adresse/components/lib/utils';
...
// Die Props-Schnittstelle wird jetzt von '@/types/types' importiert, daher hier auskommentiert
...
export function DateTimeSelectionPopup({
    ...
    // Verwendung des BookingCharacteristics Interfaces für Typsicherheit
    const characteristics: BookingCharacteristics = useMemo(
        ...
        if (isOpen) {
            ...
            if (currentCalendarMode === 'single') {
                ...
                const fromDate = (initialDateRange?.from && isValid(initialDateRange.from))
                    ? initialDateRange.from
                    : defaultToday;
                const toDate = (initialDateRange?.to && isValid(initialDateRange.to) && initialDateRange.to >= fromDate)
                    ? initialDateRange.to
                    : fromDate;
            ...
            const initialTimeValue = initialTime || "09:00";
    ...
    const handleConfirmClick = () => {
        ...
        if (durationInput !== '' && (isNaN(parsedDuration) || parsedDuration <= 0)) {
            setDurationError('Eine gültige Dauer ist erforderlich.');
            return;
        }
            ...
            <div className={`bg-white rounded-xl shadow-2xl w-full p-6 md:p-8 relative ${showTwoColumnLayout ? 'max-w-4xl' : 'max-w-md'}`}>
                        ...
                        <div className="w-full md:w-auto md:min-w-[280px] p-6 border border-gray-200 rounded-lg bg-gray-50 flex flex-col justify-between space-y-4">
                            ...
                            <div className="w-full space-y-3">
                                <button
                                    ...
                                    className="w-full bg-[#14ad9f] text-white font-bold py-3 px-4 rounded-lg hover:bg-[#129a8f] transition-colors text-base"
                ...
                {(!showTwoColumnLayout) && (
                    <div className="mt-6 pt-4 border-t border-gray-200">
                        <button
                            onClick={handleConfirmClick}
                            className="w-full bg-[#14ad9f] text-white font-semibold py-2.5 px-4 rounded-lg hover:bg-[#129a8f] transition-colors text-sm"
                            disabled={isConfirmButtonDisabled}
                        >
