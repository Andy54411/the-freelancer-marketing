...
} from '../../../../auftrag/get-started/[unterkategorie]/adresse/components/DateTimeSelectionPopup';
  ...
  const handleDateTimeConfirm: DateTimeSelectionPopupProps['onConfirm'] = useCallback(
    (selection, time, duration) => {
      ...
      if (selection instanceof Date) {
        ...
        if (
          tempSelection.from instanceof Date &&
          (tempSelection.to === undefined || tempSelection.to instanceof Date)
        ) {
      ...
      if (typeof time === 'string') {
        ...
        // Behandle null als undefined, falls es vorkommt
        finalTime = undefined;
      }
    ...
    [setSelectedDateRange, setSelectedTime, setDurationInput]
  ); // Abhängigkeiten hinzufügen

  return (
    <div className="p-4 border rounded-md bg-gray-50">
      <h4 className="text-lg font-semibold text-gray-700 mb-3 flex items-center">
        <FiCalendar className="mr-2" /> Wann soll der Task erledigt werden? *
      </h4>
      ...
      {durationInput && (
        <p className="mt-1 text-gray-700 text-sm">Dauer: {durationInput} Stunden</p>
      )}
