    ...
    return (
        <>
            <div className="p-4 border rounded-md bg-gray-50">
                <h4 className="text-lg font-semibold text-gray-700 mb-3 flex items-center"><FiMapPin className="mr-2" /> 1. Ort des Tasks *</h4>
                    ...
                    <div className="mb-4 space-y-2">
                        {userProfile.savedAddresses.map(addr => (
                            <label key={addr.id} className="flex items-center text-gray-700 cursor-pointer">
                                <Input type="radio" name="jobAddress" value={addr.id} checked={useSavedAddress === addr.id} onChange={() => { setUseSavedAddress(addr.id); setNewAddressDetails(null); }} className="h-4 w-4 text-[#14ad9f] border-gray-300 focus:ring-[#14ad9f]" />
                                <span className="ml-2">{addr.name} ({addr.line1}, {addr.postal_code} {addr.city})</span>
                            </label>
                        ))}
                        <label className="flex items-center text-gray-700 cursor-pointer">
                            <Input type="radio" name="jobAddress" value="new" checked={useSavedAddress === 'new'} onChange={() => setUseSavedAddress('new')} className="h-4 w-4 text-[#14ad9f] border-gray-300 focus:ring-[#14ad9f]" />
                            <span className="ml-2">Neue Adresse eingeben</span>
                        </label>
                    </div>
                ) : (
                    <div className="mb-4"><p className="text-sm text-gray-600">Bitte geben Sie eine neue Adresse ein.</p></div>
                )}
                {useSavedAddress === 'new' && (
                        ...
                        {isMapsLoaded && (
                            <div>
                                ...
                                <Autocomplete onLoad={onLoadAutocomplete} onPlaceChanged={onPlaceChangedAutocomplete}>
                                    <Input id="google-address-autocomplete" type="text" placeholder="Straße, PLZ, Ort..." className="w-full mt-1" />
                                </Autocomplete>
                            </div>
                        )}
            ...
            {activePostalCode && selectedSubcategory && (
                <div className="p-4 border-t mt-4">
                    <h4 className="text-lg font-semibold text-gray-700 mb-4">2. Wähle einen Tasker aus</h4>
                    {loadingProfiles && (<div className="flex items-center justify-center py-4"><FiLoader className="animate-spin mr-2" /> Lade verfügbare Tasker...</div>)}
                    {error && <p className="text-sm text-red-500 text-center py-4">{error}</p>}
                    {!loadingProfiles && !error && companyProfiles.length === 0 && (<p className="text-sm text-gray-500 text-center py-4">Für diese Postleitzahl wurden keine Tasker gefunden.</p>)}
