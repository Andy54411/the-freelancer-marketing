...
export default function CompanyCard({
    company, ratingInfo, isExpanded, onToggleDescription, onSelectProvider, onShowProfile, onOpenDatePicker, isSelected,
}: CompanyCardProps) {
    return (
        <div
            onClick={onSelectProvider}
            className={`rounded-xl border p-4 shadow-md transition-all duration-300 bg-white cursor-pointer hover:border-[#14ad9f] ${isSelected ? 'ring-2 ring-offset-2 ring-[#14ad9f] border-[#14ad9f]' : 'border-gray-200'
                }`}
        >
            <div className="flex flex-col sm:flex-row gap-4">
                <div className="flex-shrink-0 flex flex-col items-center sm:w-1/4 space-y-2">
                    ...
                    />
                    <p className="text-lg text-center font-bold text-[#14ad9f] pt-2">{company.hourlyRate ? `${Number(company.hourlyRate).toFixed(2)} €/Std.` : 'k.A.'}</p>
                </div>
