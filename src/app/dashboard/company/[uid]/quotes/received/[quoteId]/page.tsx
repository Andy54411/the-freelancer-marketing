...
export default function ReceivedQuoteDetailPage() {
  ...
  const fetchQuoteDetail = async () => {
    try {
      ...
      console.log('[Frontend] Fetching received quote detail for:', params.quoteId);
      ...
      if (response.ok) {
        ...
        if (data.success && data.quotes) {
          const foundQuote = data.quotes.find((q: any) => q.id === params.quoteId);
          if (foundQuote) {
            setQuote(foundQuote);
          } else {
  ...
  useEffect(() => {
    if (firebaseUser && params.uid && params.quoteId) {
      fetchQuoteDetail();
    }
  }, [firebaseUser, params.uid, params.quoteId]);
  ...
  const handleQuoteAction = async (action: 'accept' | 'decline') => {
    ...
    try {
      ...
      const response = await fetch(
        `/api/company/${params.uid}/quotes/received/${params.quoteId}/action`,
        {
          method: 'POST',
          headers: {
    ...
    <div className="space-y-6">
      ...
      <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-6">
        <div className="flex items-center justify-between mb-4">
          <button
            onClick={() => router.push(`/dashboard/company/${params.uid}/quotes/received`)}
      ...
      {quote.hasResponse && quote.response ? (
        <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-6">
          ...
          {/* Kontaktdaten-Austausch für angenommene Angebote */}
