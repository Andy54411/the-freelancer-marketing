...
export default function EditQuotePage() {
  ...
  const [originalQuote, setOriginalQuote] = useState<QuoteType | null>(null);
  ...
  // EDIT-SPEZIFISCH: Angebot laden
  useEffect(() => {
    const loadExistingQuote = async () => {
      if (!quoteId || !uid) return;
      ...
      try {
        ...
        setFormData({
          ...
          customerAddress: (() => {
            // Handle both string and object formats for customerAddress
            if (typeof quote.customerAddress === 'string') {
              return quote.customerAddress;
          ...
          quoteDate: quote.date ? format(new Date(quote.date), 'yyyy-MM-dd') : '',
          deliveryDate: quote.deliveryDate ? format(new Date(quote.deliveryDate), 'yyyy-MM-dd') : '',
          servicePeriod: quote.servicePeriod || '',

          // Texte und Beschreibungen
          headTextHtml: quote.headTextHtml || quote.description || '[%VOLLEANREDE%],\n\n[%QUOTE_INTRO_TEXT%]',
          footerText: quote.footerText || '[%QUOTE_CLOSING_TEXT%]<br>[%BESTREGARDS%]<br>[%KONTAKTPERSON%]',
          notes: quote.notes || '',

          // Geschäftskonditionen
          currency: quote.currency || 'EUR',
          internalContactPerson: quote.internalContactPerson || '',
          deliveryTerms: quote.deliveryTerms || '',
          paymentTerms: quote.paymentTerms || '',
          taxRule: quote.taxRule || TaxRuleType.DE_TAXABLE,
        ...
        // Skonto-States setzen
        setSkontoEnabled(quote.skontoEnabled || false);
        setSkontoDays(quote.skontoDays);
  ...
  useEffect(() => {
    ...
    const currentQuoteDate = formData.quoteDate || new Date().toISOString().split('T')[0];
    if (currentQuoteDate && paymentDays > 0) {
  ...
  useEffect(() => {
    const currentQuoteDate = formData.quoteDate || new Date().toISOString().split('T')[0];
    if (currentQuoteDate && !formData.validUntil) {
  ...
  // EDIT-SPEZIFISCH: Angebot aktualisieren
  const handleUpdateQuote = async (asDraft = false) => {
    if (!originalQuote) {
      toast.error('Originales Angebot nicht gefunden');
      return;
    }
    ...
    try {
      ...
      // Prepare update data - ALLE FORMULARFELDER VOLLSTÄNDIG MAPPEN
      const updateData: Partial<QuoteType> = {
        // Customer data - alle Kundenfelder
      ...
      toast.error(`Fehler beim Speichern des Angebots: ${errorMessage}`);
    }
  };
  ...
  // EDIT-SPEZIFISCHER GUARD: Prüfe ob Quote geladen wurde
  if (loadingQuote) {
    return (
      <div className="min-h-screen bg-gray-50">
        <div className="flex justify-center items-center min-h-[400px]">
          <div className="text-center">
            ...
            <h2 className="text-xl font-semibold text-gray-900 mb-2">Angebot wird geladen...</h2>
      ...
      <div className="min-h-screen bg-gray-50">
        <div className="flex justify-center items-center min-h-[400px]">
          <div className="text-center">
            ...
            <p className="text-gray-600 mb-4">Das angeforderte Angebot konnte nicht geladen werden.</p>
            <Button onClick={() => router.push(`/dashboard/company/${uid}/finance/quotes`)}>
  ...
  const handleSubmit = async (asDraft = true) => {
    // EDIT-SPEZIFISCH: Rufe die Update-Funktion auf statt Create
    await handleUpdateQuote(asDraft);
  };

  // ALTE CREATE-FUNKTIONALITÄT (für Referenz erhalten, falls benötigt)
  const handleOriginalSubmit = async (asDraft = true) => {
    if (loading) return;
    setLoading(true);
    try {
      // Validation
