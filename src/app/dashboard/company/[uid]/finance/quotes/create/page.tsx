...
export default function CreateQuotePage() {
  ...
  const [formData, setFormData] = useState<CreateQuoteFormData>({
    ...
    '[%VOLLEANREDE%],\n\n[%QUOTE_INTRO_TEXT%]',
    footerText:
    '[%QUOTE_CLOSING_TEXT%]<br>[%BESTREGARDS%]<br>[%KONTAKTPERSON%]',
    notes: '',
    currency: 'EUR',
      ...
      try {
        ...
        if (companySnap.exists()) {
          ...
          if (numbering) {
            ...
            const quoteFormat = numbering.format ? numbering.format.replace('RE-', 'AN-') : 'AN-%NUMBER';
  ...
  const buildPreviewData = (): PreviewTemplateData => {
    ...
    const data: PreviewTemplateData = {
      ...
      formatDateDE(formData.quoteDate) :
      formatDateDE(new Date()),
      quoteDate: formData.quoteDate ?
      formatDateDE(formData.quoteDate) :
      formatDateDE(new Date()),
      invoiceDate: formData.quoteDate ?
      formatDateDE(formData.quoteDate) :
      formatDateDE(new Date()), // Fallback für Template-Kompatibilität
      ...
      formatDateDE(formData.deliveryDate) :
      undefined,
