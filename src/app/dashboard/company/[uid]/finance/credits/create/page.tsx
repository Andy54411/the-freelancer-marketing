...
import { useNextStornoNumber } from '@/hooks/useNextStornoNumber';
...
export default function CreateStornoPage() {
  ...
  const { nextNumber: nextStornoNumber, isLoading: nextNumberLoading, error: nextNumberError, refresh: _refreshNextNumber } = useNextStornoNumber(uid);
  ...
  }, [nextStornoNumber, nextNumberLoading, nextNumberError, uid]);
  ...
  const [numberingFormat, setNumberingFormat] = useState('ST-%NUMBER');
  const [nextNumber, setNextNumber] = useState(1000);

  // Originalrechnung für Stornorechnung (GoBD-konform)
  const [selectedOriginalInvoice, setSelectedOriginalInvoice] = useState<InvoiceData | null>(null);
  ...
  const [formData, setFormData] = useState<CreateInvoiceFormData>({
    ...
    headTextHtml: '[%VOLLEANREDE%],\n\nhiermit stornieren wir die folgende Rechnung:',
    footerText:
    'Wir bitten um Beachtung dieser Stornorechnung. Bei Rückfragen stehen wir Ihnen gerne zur Verfügung.<br>Mit freundlichen Grüßen<br>[%KONTAKTPERSON%]',
    notes: '',
    currency: 'EUR',
  ...
  // Lade verfügbare Rechnungen für Stornorechnung-Verknüpfung
  useEffect(() => {
    const loadInvoices = async () => {
      if (!uid) return;
      setLoadingInvoices(true);
      try {
        const invoices = await InvoiceService.getInvoicesByCompany(uid);
        // Filtere nur normale Rechnungen (keine Stornos, keine stornierten)
        const availableForStorno = invoices.filter(inv => 
          !inv.isStorno && 
        ...
        availableForStorno.sort((a, b) => {
          const dateA = a.date ? new Date(a.date).getTime() : 0;
          const dateB = b.date ? new Date(b.date).getTime() : 0;
          return dateB - dateA;
        });
        setAvailableInvoices(availableForStorno);
      } catch (error) {
  ...
  useEffect(() => {
    const loadNumberingSettings = async () => {
      ...
      try {
        ...
        const stornoSequence = result.find((seq) => seq.type === 'Storno');

        if (stornoSequence) {
          setNumberingFormat(stornoSequence.format || 'ST-{number}');
          setNextNumber(stornoSequence.nextNumber || 1);
    ...
    const data: PreviewTemplateData = {
      ...
      items: items.map((it) => {
        ...
        return {
          ...
          quantity: qty,
          unitPrice: -Math.abs(unit), // STORNO: Negativ!
          total: -Math.abs(lineTotalNet), // STORNO: Negativ!
          taxRate: undefined,
      ...
      tax: (() => {
        ...
        // Bei Netto-Anzeige keine Steuer anzeigen - STORNO: Negativ!
        return showNet ? 0 : -Math.abs(vat);
      })(),
      total: (() => {
        const isReverseCharge =
        formData.taxRule?.includes('REVERSE') ||
        ...
        formData.taxRule === TaxRuleType.NON_EU_OUT_OF_SCOPE;
        // STORNO: Alle Beträge negativ!
        if (isReverseCharge || isTaxExempt) return -Math.abs(subtotal);
        // Bei Netto-Anzeige nur Netto-Summe zeigen
        return showNet ? -Math.abs(subtotal) : -Math.abs(grandTotal);
      })(),
      vatRate: (() => {
  ...
  const handleOpenSendModal = () => {
    ...
    // Create temporary storno data for modal preview
    const tempInvoiceData: InvoiceType = {
      id: '',
      companyId: uid,
      invoiceNumber: formData.title || `ST-${nextNumber}`,
      number: formData.title || `ST-${nextNumber}`,
      ...
      // Stornorechnungsbeschreibung
      description: formData.title || `Stornorechnung ${nextNumber}`,

      // Finanzielle Daten - STORNO: Alle Beträge negativ!
      amount: -Math.abs(subtotal), // Nettobetrag (negativ)
      tax: -Math.abs(vat), // Steuerbetrag (negativ)
      total: -Math.abs(grandTotal), // Gesamtbetrag (negativ)
      ...
      // Items and totals - STORNO: Alle Beträge werden NEGATIV!
      items: items.map((item) => ({
        id: item.id,
        description: item.description,
        quantity: item.quantity,
        unitPrice: -Math.abs(item.unitPrice), // Negativ für Storno!
        total: -Math.abs(item.total), // Negativ für Storno!
  ...
  const handleSubmit = async (asDraft = true) => {
    ...
    try {
      ...
      // STORNO: Generiere Stornonummer DYNAMISCH beim Speichern!
      // Verhindert Race Conditions bei gleichzeitiger Storno-Erstellung

      let finalStornoNumber = '';
      ...
      try {
        ...
        // Verwende NumberSequenceService für Storno-Nummern
        const numberResult = await NumberSequenceService.getNextNumberForType(uid, 'Storno');
        finalStornoNumber = numberResult.formattedNumber;
        finalSequentialNumber = numberResult.number; // 'number' statt 'sequentialNumber'

      } catch (numberError) {
        console.error('❌ Fehler bei Stornonummer-Generierung:', {
          error: numberError,
        ...
        finalStornoNumber = `ST-${Date.now().toString().slice(-4)}`;
      ...
      const stornoData = {
        // Basic Storno Info - ID will be set by Firestore
        id: '', // This will be set by Firestore when saving
        companyId: uid,
        invoiceNumber: finalStornoNumber, // Atomisch generierte Stornonummer
        stornoNumber: finalStornoNumber, // Stornonummer
        number: finalStornoNumber, // Atomisch generierte Stornonummer
        sequentialNumber: finalSequentialNumber, // Korrekte sequentielle Nummer
        status: asDraft ? 'draft' : 'finalized',
        documentType: 'cancellation', // Kennzeichnung als Stornorechnung
        ...
        documentNumber: formData.title || `ST-${nextNumber}`,
        reference: formData.customerOrderNumber || undefined,
        ...
        // STORNO: Items mit negativen Beträgen
        items: items.
        ...
        // Originalrechnung-Verknüpfung (GoBD-konform)
        originalInvoiceId: selectedOriginalInvoice?.id ?? null,
        originalInvoiceNumber: selectedOriginalInvoice?.invoiceNumber ?? null,
        originalInvoiceDate: selectedOriginalInvoice?.date ?? null,
      ...
      const cleanStornoData = JSON.parse(
        JSON.stringify(stornoData, (key, value) => {
          return value === undefined ? null : value;
        })
      );

      // Restore Date objects that got converted to strings
      cleanStornoData.createdAt = new Date(cleanStornoData.createdAt);
      cleanStornoData.updatedAt = new Date(cleanStornoData.updatedAt);

      // Save Storno using FirestoreInvoiceService - TARGET: invoices subcollection (Stornos are stored with invoices)
      const createdStornoId = await InvoiceService.saveInvoice(cleanStornoData);
      ...
      try {
        ...
        console.warn('Service usage tracking failed (non-critical):', trackingError);
      }

      // Inventory Management - Bei Storno: Bestand zurückbuchen!
      try {
        ...
        if (inventoryItems.length > 0) {
          // TODO: Bestandsrückbuchung bei Storno implementieren
        }
      } catch (reserveErr: unknown) {
        console.error('Inventory reservation failed:', reserveErr);
        // Continue anyway - inventory reservation is not critical for storno creation
      }

      // Storno number sequence is now automatically managed by NumberSequenceService

      toast.success(asDraft ? 'Stornorechnung als Entwurf gespeichert' : 'Stornorechnung erstellt');

      // E-RECHNUNG GENERIERUNG NACH SPEICHERN (falls aktiviert)
      if (!asDraft && eInvoiceEnabled && createdStornoId) {
        try {
          const { AutoEInvoiceService } = await import('@/services/autoEInvoiceService');

          // Lade die gerade gespeicherte Stornorechnung
          const savedStorno = await InvoiceService.getInvoiceById(uid, createdStornoId);
          if (savedStorno) {
            ...
            if (eInvoiceResult) {
              // Update die Stornorechnung mit den E-Invoice-Daten
              const updateData = {
                eInvoiceData: {
              ...
              await InvoiceService.updateInvoice(createdStornoId, updateData);
      ...
      // 🚨 CRITICAL: Update stornoData with the newly created ID from Firestore
      stornoData.id = createdStornoId;

      // Open send document modal instead of navigating
      setCreatedDocument(stornoData);
      setShowSendDocumentModal(true);
    } catch (e) {
      console.error('❌ CRITICAL ERROR in handleSubmit:', e);
      toast.error('Stornorechnung konnte nicht gespeichert werden');
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6 space-y-6">
        ...
        <div className="flex items-center justify-between py-4 border-b border-gray-200">
          ...
          <div className="flex items-center">
            <div>
              <h2 className="text-2xl font-bold text-gray-900">Neue Stornorechnung</h2>
            </div>
          </div>
      ...
      {/* Originalrechnung auswählen - WICHTIG: Muss zuerst ausgewählt werden */}
      <Card className="border-2 border-[#14ad9f]/30 bg-linear-to-br from-[#14ad9f]/5 to-white">
        <CardHeader>
          <CardTitle className="flex items-center">
            <FileText className="h-5 w-5 mr-2 text-[#14ad9f]" />
            Originalrechnung verknüpfen
            <span className="ml-2 text-xs font-normal text-gray-500">(GoBD-konform)</span>
          </CardTitle>
        </CardHeader>
        <CardContent className="space-y-4">
          <div className="text-sm text-gray-600 mb-4">
            Wählen Sie die Rechnung aus, die storniert werden soll. Die Positionen und Kundendaten werden automatisch übernommen.
          </div>
          
          <div className="space-y-2">
            <Label className="text-sm font-medium text-gray-700">
              Zu stornierende Rechnung <span className="text-red-500">*</span>
            </Label>
            <Popover open={invoiceSearchOpen} onOpenChange={setInvoiceSearchOpen}>
              <PopoverTrigger asChild>
                <Button
                  ...
                  className="w-full justify-between h-auto min-h-11 py-2"
                >
                  {selectedOriginalInvoice ? (
                    <div className="flex flex-col items-start text-left">
                      <span className="font-medium">{selectedOriginalInvoice.invoiceNumber}</span>
                      <span className="text-xs text-gray-500">
                        {selectedOriginalInvoice.customerName} - {selectedOriginalInvoice.date ? new Date(selectedOriginalInvoice.date).toLocaleDateString('de-DE') : ''} - {((selectedOriginalInvoice.total ?? 0)).toLocaleString('de-DE', { style: 'currency', currency: 'EUR' })}
                      </span>
                    </div>
                  ) : (
                    <span className="text-gray-500">Rechnung auswählen...</span>
                  )}
                  <ChevronsUpDown className="ml-2 h-4 w-4 shrink-0 opacity-50" />
                </Button>
              </PopoverTrigger>
                    ...
                    {availableInvoices
                      ...
                      .map((invoice) => (
                        <CommandItem
                          ...
                          onSelect={() => {
                            // Setze die ausgewählte Rechnung
                            setSelectedOriginalInvoice(invoice);
                            setInvoiceSearchOpen(false);
                            setInvoiceSearchQuery('');
                            
                            // Übernehme Kundendaten von der Originalrechnung
                            setFormData(prev => ({
                              ...prev,
                              ...
                              headTextHtml: `[%VOLLEANREDE%],\n\nhiermit stornieren wir die Rechnung ${invoice.invoiceNumber} vom ${invoice.date ? new Date(invoice.date).toLocaleDateString('de-DE') : ''}:`,
                              currency: invoice.currency ?? 'EUR',
                              taxRule: invoice.taxRule ?? TaxRuleType.DE_TAXABLE,
                            }));
                            
                            // Übernehme Positionen mit negativen Beträgen
                            if (invoice.items && invoice.items.length > 0) {
                              const stornoItems: QuoteItem[] = invoice.items.map((item, index) => ({
                                id: crypto.randomUUID ? crypto.randomUUID() : `storno-${index}-${Date.now()}`,
                                description: item.description ?? '',
                                quantity: Math.abs(item.quantity ?? 1) * -1, // Negativ für Storno
                                unitPrice: Math.abs(item.unitPrice ?? 0),
                                total: Math.abs(item.total ?? 0) * -1, // Negativ für Storno
                                unit: item.unit,
                                taxRate: item.taxRate,
                                category: item.category,
                              }));
                              setItems(stornoItems);
                            }
                            
                            toast.success(`Rechnung ${invoice.invoiceNumber} wurde verknüpft`);
                          }}
                        >
                          <Check
                            className={cn(
                              "mr-2 h-4 w-4",
                              selectedOriginalInvoice?.id === invoice.id ? "opacity-100" : "opacity-0"
                            )}
                          />
                          <div className="flex flex-col flex-1">
                            <div className="flex items-center justify-between">
                              ...
                              <span className="text-sm text-gray-600">
                                {((invoice.total ?? 0)).toLocaleString('de-DE', { style: 'currency', currency: 'EUR' })}
                              </span>
                            </div>
                            <div className="flex items-center justify-between text-xs text-gray-500">
                              <span>{invoice.customerName}</span>
                              <span>{invoice.date ? new Date(invoice.date).toLocaleDateString('de-DE') : ''}</span>
                            </div>
                          </div>
                        </CommandItem>
                      ))}
          ...
          {selectedOriginalInvoice && (
            <div className="p-4 bg-[#14ad9f]/10 rounded-lg border border-[#14ad9f]/20">
              <div className="flex items-start gap-3">
                <Info className="h-5 w-5 text-[#14ad9f] mt-0.5 shrink-0" />
                <div className="text-sm">
                  <p className="font-medium text-gray-900 mb-1">Verknüpfte Rechnung</p>
                  <p className="text-gray-600">
                    Diese Stornorechnung bezieht sich auf <strong>{selectedOriginalInvoice.invoiceNumber}</strong> vom {selectedOriginalInvoice.date ? new Date(selectedOriginalInvoice.date).toLocaleDateString('de-DE') : ''}.
                    Die Positionen wurden mit negativen Beträgen übernommen.
                  </p>
                </div>
                <Button
                  variant="ghost"
      ...
      <Card>
        ...
        <CardContent className="space-y-6">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
            ...
            {/* Stornoinformationen Sektion - Rechts */}
            <div className="space-y-4">
              <div>
                <h3 className="text-lg font-semibold text-gray-900 mb-4">Stornoinformationen</h3>

                {/* 2x2 Grid für Stornofelder */}
                <div className="grid grid-cols-2 gap-4 mb-4">
                  {/* Stornodatum */}
                  <div className="space-y-2">
                    <div className="flex items-center gap-1">
                      <Label className="text-sm font-medium text-gray-700">Stornodatum</Label>
                      <span className="text-red-500">*</span>
                    </div>
                    <Input
                        type="date"
                  ...
                  <div className="space-y-2">
                    <div className="flex items-center gap-1">
                      <Label className="text-sm font-medium text-gray-700">Stornonummer</Label>
                      <span className="text-red-500">*</span>
                    </div>
                    <div className="relative">
                      <Input
                          ...
                          value={
                          formData.title || (
                          nextStornoNumber && !nextNumberLoading && !nextNumberError ?
                          nextStornoNumber :
                          '')
                          }
                          ...
                          readOnly
                          title="Die Stornonummer wird beim Speichern automatisch vergeben, um doppelte Nummern zu verhindern." />
      ...
      <SendDocumentModal
          ...
          companyId={uid}
          redirectAfterAction={`/dashboard/company/${uid}/finance/credits`}
          onSend={async (method, options) => {
            try {
              ...
              if (method === 'email') {
                ...
                toast.success('Stornorechnung wurde gespeichert und E-Mail-Versand wird vorbereitet');
                ...
                toast.success('Stornorechnung erfolgreich gespeichert');
              } else if (method === 'post') {
                // Post action - already handled in SendDocumentModal
                toast.success('Per Post versenden wurde bearbeitet');
              }

              // Navigate to credits list after successful action
              router.push(`/dashboard/company/${uid}/finance/credits`);
            } catch (error) {
              console.error('Error sending document:', error);
              toast.error('Fehler beim Versenden der Stornorechnung');
            }
          }} />
