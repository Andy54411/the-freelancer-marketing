...
// PDF Template System für Auftragsbestätigungen
...
// Auftragsbestätigung-spezifische Imports
...
// Use InvoiceTemplate type from @/components/finance/InvoiceTemplates (dynamic templates work for invoices, quotes and order confirmations)
...
export default function CreateOrderConfirmationPage() {
  ...
  <Card>
      ...
      <CardContent>
        ...
        onServiceAdded={(service) => {
          ...
          toast.success('Dienstleistung wurde zur Auftragsbestätigung hinzugefügt');
        }} />
  ...
  const handleQuickAddService = async () => {
    ...
    try {
      ...
      // 2. Direkt zur Auftragsbestätigung hinzufügen
      const newItem = {
      ...
      toast.success('Dienstleistung gespeichert und zur Auftragsbestätigung hinzugefügt');
    } catch (e) {
  ...
  const [emailAttachmentName, setEmailAttachmentName] = useState<string>('Auftragsbestaetigung.docx');
  ...
  // Form state für Auftragsbestätigungen
  interface CreateOrderConfirmationFormData {
    customerName: string;
  ...
  const [formData, setFormData] = useState<CreateOrderConfirmationFormData>({
    customerName: '',
    customerFirstName: '',
    customerLastName: '',
    customerNumber: '',
    ...
    headTextHtml: '[%VOLLEANREDE%],\n\n[%ORDER_CONFIRMATION_INTRO_TEXT%]',
    footerText: '[%ORDER_CONFIRMATION_CLOSING_TEXT%]<br>[%BESTREGARDS%]<br>[%KONTAKTPERSON%]',
    notes: '',
    currency: 'EUR',
  ...
  // Auftragsbestätigung-Nummerierung laden - FIXED to use NumberSequenceService
  const [numberingLoaded, setNumberingLoaded] = useState(false);

  useEffect(() => {
    if (!uid || numberingLoaded) return;

    const loadOrderConfirmationNumbering = async () => {
      try {


        // Verwende das NumberSequenceService für korrekte Auftragsbestätigung-Nummerierung
        const sequences = await NumberSequenceService.getNumberSequences(uid);
        const orderConfirmationSequence = sequences.find((seq) => seq.type === 'Auftragsbestätigung');

        if (orderConfirmationSequence) {
          // Verwende das existierende NumberSequence für Auftragsbestätigungen
          const previewNumber = NumberSequenceService.formatNumber(
            orderConfirmationSequence.nextNumber,
            orderConfirmationSequence.format
          );



          setNumberingFormat(orderConfirmationSequence.format);
          setNextNumber(orderConfirmationSequence.nextNumber);
          ...
          // Fallback: Erstelle neues Auftragsbestätigung-NumberSequence


          const fallbackNumber = 'AB-1001';
          setNumberingFormat('AB-{number}');
          setNextNumber(1001);
        ...
        console.error('Fehler beim Laden der Auftragsbestätigung-Nummerierung:', error);

        // Fallback bei Fehler
        const fallbackNumber = `AB-${Date.now().toString().slice(-4)}`;
        setFormData((prev) => ({
          ...prev,
          title: fallbackNumber
        }));
    ...
    loadOrderConfirmationNumbering();
  }, [uid]);
  ...
  useEffect(() => {
    ...
    const currentOrderConfirmationDate = formData.orderConfirmationDate;
    if (!currentOrderConfirmationDate || paymentDays <= 0) return;
    
    const orderConfirmationDate = new Date(currentOrderConfirmationDate);
    const dueDate = new Date(orderConfirmationDate);
    dueDate.setDate(orderConfirmationDate.getDate() + paymentDays);
  ...
  }, [paymentDays, formData.orderConfirmationDate]);

  // Initiale Berechnung nur beim ersten Laden
  useEffect(() => {
    const currentOrderConfirmationDate = formData.orderConfirmationDate;
    if (!currentOrderConfirmationDate) return;
    
    if (!formData.validUntil) {
      // Wenn kein Fälligkeitsdatum gesetzt ist, verwende Standard 30 Tage für Auftragsbestätigungen
      const orderConfirmationDate = new Date(currentOrderConfirmationDate);
      const validUntilDate = new Date(orderConfirmationDate);
      validUntilDate.setDate(orderConfirmationDate.getDate() + 30);

      const validUntilString = validUntilDate.toISOString().split('T')[0];
      setFormData((prev) => ({
        ...prev,
        validUntil: validUntilString,
        paymentTerms: 'Gültig für 30 Tage',
      }));
      setPaymentDays(30);
    }
  ...
  const handleValidUntilChange = useCallback((newValidUntil: string) => {
    const currentOrderConfirmationDate = formData.orderConfirmationDate;
    if (!currentOrderConfirmationDate || !newValidUntil) return;
    
    const orderConfirmationDate = new Date(currentOrderConfirmationDate);
    const dueDate = new Date(newValidUntil);
    const diffTime = dueDate.getTime() - orderConfirmationDate.getTime();
    ...
    if (diffDays > 0) {
      ...
      setFormData((prev) => ({
        ...
        paymentTerms: `Gültig für ${diffDays} Tage`,
      }));
    } else {
      setPaymentDays(0);
      setFormData((prev) => ({
  ...
  }, [formData.orderConfirmationDate]);
  ...
  // Datum als dd.mm.yyyy formatieren - KORRIGIERT!
  ...
  const buildPreviewData = (): PreviewTemplateData => {
    ...
    const data: PreviewTemplateData = {
      documentType: 'orderConfirmation' as const,
      quoteNumber: formData.title || `AB-${nextNumber}`,
      documentNumber: formData.title || `AB-${nextNumber}`,
      invoiceNumber: formData.title || `AB-${nextNumber}`, // Fallback für Template-Kompatibilität
      date: formData.orderConfirmationDate ? formatDateDE(formData.orderConfirmationDate) : formatDateDE(new Date()),
      quoteDate: formData.orderConfirmationDate ? formatDateDE(formData.orderConfirmationDate) : formatDateDE(new Date()),
      invoiceDate: formData.orderConfirmationDate ? formatDateDE(formData.orderConfirmationDate) : formatDateDE(new Date()), // Fallback für Template-Kompatibilität
  ...
  useEffect(() => {
    ...
    const data = buildPreviewData();
    if (!emailSubject) {
      setEmailSubject(`Auftragsbestätigung ${data.companyName}${data.title ? ' – ' + data.title : ''}`);
    }
    if (!emailBody) {
      setEmailBody(
        `Hallo ${data.customerName || ''},\n\n` +
        `anbei erhalten Sie Ihre Auftragsbestätigung${data.title ? ' zu: ' + data.title : ''}.` +
  ...
  const handleOpenSendModal = () => {
    ...
    const tempQuoteData: QuoteType = {
      ...
      // Auftragsbestätigungsbeschreibung
      title: formData.title || `Auftragsbestätigung ${nextNumber}`,
      description: formData.title || `Auftragsbestätigung ${nextNumber}`,
  ...
  const handleSubmit = async (asDraft = true) => {
    ...
    try {
      ...
      const quoteData = {
        ...
        number: formData.title || `AB-${nextNumber}`,
        sequentialNumber: nextNumber,
        status: asDraft ? 'draft' : 'finalized',

        // Dates - ALLE Datumswerte aus dem Formular
        date: formData.orderConfirmationDate ?
        new Date(formData.orderConfirmationDate).toISOString().split('T')[0] :
        new Date().toISOString().split('T')[0],
        issueDate: formData.orderConfirmationDate ?
        new Date(formData.orderConfirmationDate).toISOString().split('T')[0] :
        new Date().toISOString().split('T')[0],
        validUntilDate: formData.validUntil ?
        new Date(formData.validUntil).toISOString().split('T')[0] :
        new Date(Date.now() + 30 * 24 * 60 * 60 * 1000).toISOString().split('T')[0],
        orderConfirmationDate: formData.orderConfirmationDate || '',
        validUntil: formData.validUntil || '',
      ...
      // ✅ Order confirmation number sequence is now automatically managed by NumberSequenceService
      // ❌ REMOVED: setNextNumber manipulation - causes race conditions!
      // NumberSequenceService handles all numbering automatically

      toast.success(asDraft ? 'Auftragsbestätigung als Entwurf gespeichert' : 'Auftragsbestätigung erstellt');
      ...
      toast.error('Auftragsbestätigung konnte nicht gespeichert werden');
    } finally {
      setLoading(false);
    }
  };
      ...
      <header className="w-full">
        <div className="flex items-center justify-between py-4 border-b border-gray-200">
          ...
          <div className="flex items-center">
            <div>
              <h2 className="text-2xl font-bold text-gray-900">Neue Auftragsbestätigung</h2>
            </div>
          </div>
      ...
      <Card>
        ...
        <CardContent className="space-y-6">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
            ...
            {/* Auftragsbestätigungs-Informationen Sektion - Rechts */}
            <div className="space-y-4">
              <div>
                <h3 className="text-lg font-semibold text-gray-900 mb-4">Auftragsbestätigungs-Informationen</h3>

                {/* 2x2 Grid für Auftragsbestätigungsfelder */}
                <div className="grid grid-cols-2 gap-4 mb-4">
                  {/* Auftragsbestätigungsdatum */}
                  <div className="space-y-2">
                    <div className="flex items-center gap-1">
                      <Label className="text-sm font-medium text-gray-700">Auftragsbestätigungsdatum</Label>
                      <span className="text-red-500">*</span>
                    </div>
                    <Input
                      type="date"
                      value={formData.orderConfirmationDate || new Date().toISOString().split('T')[0]}
                      onChange={(e) => {
                        setFormData((prev) => ({ ...prev, orderConfirmationDate: e.target.value }));
                      }}
                      required />
                  ...
                  <div className="space-y-2">
                    <div className="flex items-center gap-1">
                      <Label className="text-sm font-medium text-gray-700">Auftragsbestätigungsnummer</Label>
                      <span className="text-red-500">*</span>
                    </div>
                    <div className="relative">
                      <Input
                ...
                {/* Gültig bis - Ganze Zeile */}
                <div className="space-y-2">
                  <Label className="text-sm font-medium text-gray-700">Gültig bis</Label>
                  <div className="flex items-center gap-2">
                    <Input
                      type="date"
      ...
      <Card>
        ...
        <CardContent>
          <Textarea
            ...
            placeholder="Interne Notizen (werden nicht in der Auftragsbestätigung angezeigt)..."
            rows={3} />
      ...
      <SendDocumentModal
        ...
        onSend={async (method, options) => {
          try {
            ...
            // Navigate to order confirmations list after successful send (dynamic routing fix)
            router.push(`/dashboard/company/${uid}/finance/order-confirmations`);
          } catch (error) {
            console.error('Error sending document:', error);
            toast.error('Fehler beim Versenden der Auftragsbestätigung');
          }
        }} />
