  ...
  const handleBankTransfer = async (transferData: any) => {
    try {
      ...
      if (result.success) {
        ...
        alert(
          `Für diese Überweisung ist eine Web Form Authentifizierung erforderlich.\n\n${result.message}\n\nBitte kontaktieren Sie den Support für weitere Informationen.`
        );
      ...
      alert(
        `Fehler bei der Überweisung: ${error instanceof Error ? error.message : 'Unbekannter Fehler'}`
      );
  ...
  const handleOpenConnectedDocumentsModal = (transaction: Transaction) => {
    ...
    const connectedDocuments = transaction.linkedInvoices.map((invoice, index) => {
      ...
      const relevantLink = transactionLinks.find(
        link => link.transactionId === transaction.id && link.documentId === invoice.documentId
      );
        ...
        prevTransactions.map(tx =>
          tx.id === transactionId
            ? {
                ...
                bookingStatus:
                  tx.verknuepfungen.filter(id => id !== documentId).length > 0 ? 'booked' : 'open',
              }
            : tx
        )
      );
              ...
              <tbody className="bg-white divide-y divide-gray-100">
                {paginatedTransactions.map(transaction => (
                  <tr key={transaction.id} className="hover:bg-gray-50 h-12">
                    ...
                    <td className="px-2 py-2 text-center">
                      {(() => {
                        ...
                        if (transaction.verknuepfungen.length > 0) {
                          ...
                          const relevantLinks = transactionLinks.filter(
                            link => link.transactionId === transaction.id
                          );
                          ...
                          if (relevantLinks.length > 0) {
                            ...
                            offenerBetrag =
                              transaction.betrag + stornoRechnungen - normalRechnungen;
                    ...
                    >
                      <div
                        className={`${transaction.verknuepfungen.length === 0 ? 'flex justify-center' : 'flex flex-col gap-1'}`}
                      >
                                  ...
                                  <button
                                    onClick={e => {
                                      ...
                                      const hasMultipleDocuments =
                                        ...
                                        transaction.linkedInvoices.length > 1;
                                      ...
                                      const shouldOpenModal =
                                        !istVollstaendigBeglichen || hasMultipleDocuments;
                    ...
                    {(() => {
                      ...
                      const shouldShowActions =
                        transaction.verknuepfungen.length === 0 || !istVollstaendigBeglichen;
