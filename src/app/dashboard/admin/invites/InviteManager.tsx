...
export default function InviteManager({ initialCodes }: InviteManagerProps) {
    ...
    const handleGenerateAndSendCode = async () => {
        ...
        try {
            const createInviteCodeCallable = httpsCallable<CreateInviteData, CreateInviteResult>(functions, 'createInviteCode');
            ...
            if (result.data.success && result.data.inviteCode) {
                ...
                if (result.data.emailSent) {
                    setSuccessMessage(`Code ${result.data.inviteCode} erfolgreich erstellt und an ${recipientEmail} gesendet.`);
                } else {
                    setSuccessMessage(`Code ${result.data.inviteCode} erfolgreich erstellt. (Es wurde keine E-Mail gesendet, da keine Adresse angegeben wurde.)`);
                }
    ...
    const handleDeleteCode = async (codeId: string) => {
        ...
        try {
            const deleteInviteCodeCallable = httpsCallable<DeleteInviteData, DeleteInviteResult>(functions, 'deleteInviteCode');
            ...
            setError("Ein interner Fehler ist beim Löschen aufgetreten. Bitte überprüfen Sie die Server-Logs.");
        } finally {
                    ...
                    <TableBody>
                        {codes.length > 0 ? codes.map((c) => (
                            <TableRow key={c.id}>
                                ...
                                <TableCell><Badge variant={c.role === 'master' ? 'default' : 'secondary'}>{c.role}</Badge></TableCell>
                                <TableCell>{c.createdAt.toLocaleDateString()}</TableCell>
                                <TableCell className="text-right">
                                    <AlertDialog>
                                        <AlertDialogTrigger asChild>
                                            <Button variant="ghost" size="icon" disabled={deletingCodeId === c.id} title="Löschen">
                                        ...
                                        <AlertDialogContent>
                                            ...
                                            <AlertDialogFooter>
                                                ...
                                                <AlertDialogAction onClick={() => handleDeleteCode(c.id)}>Löschen</AlertDialogAction>
                                            </AlertDialogFooter>
                                        </AlertDialogContent>
                                    </AlertDialog>
                                </TableCell>
                            ...
                            <TableRow>
                                <TableCell colSpan={4} className="text-center">Keine unbenutzten Einladungscodes gefunden.</TableCell>
                            </TableRow>
                        )}
                    </TableBody>
                </Table>
            ...
            <CardFooter>
                <Dialog open={isDialogOpen} onOpenChange={(open) => {
                    ...
                    <DialogTrigger asChild>
                        <Button disabled={!canCreateInvites} title={!canCreateInvites ? "Sie haben nicht die erforderliche Rolle (master/support)." : ""}>
                    ...
                    <DialogContent className="sm:max-w-[425px]">
                        <DialogHeader>
                            ...
                            <DialogDescription>
                                Geben Sie die E-Mail-Adresse des Empfängers und die gewünschte Rolle an. Der Code wird direkt versendet.
                            </DialogDescription>
                        </DialogHeader>
                        <div className="grid gap-4 py-4">
                            ...
                            <div className="grid grid-cols-4 items-center gap-4">
                                ...
                                <Select value={roleToCreate} onValueChange={(value: 'support' | 'master') => setRoleToCreate(value)}>
