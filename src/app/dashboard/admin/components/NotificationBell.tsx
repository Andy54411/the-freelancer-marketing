            ...
            <DropdownMenuContent align="end" className="w-80">
                ...
                {notifications.length > 0 ? (
                    notifications.map(notif => (
                        <DropdownMenuItem key={notif.id} onSelect={() => handleNotificationClick(notif)} className="cursor-pointer flex items-start gap-3">
                            <FiMail className="mt-1 h-4 w-4 text-gray-500 shrink-0" />
                            <p className="text-sm font-medium whitespace-normal flex-1">{notif.message} <span className="text-xs text-gray-400 ml-1">{formatTimeAgo(notif.createdAt)}</span></p>
                        </DropdownMenuItem>
                    ))
