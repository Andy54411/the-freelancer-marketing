...
async function listUserDocuments(
  companyId: string
): Promise<{ files: { name: string; url: string }[] }> {
  try {
    const isEmulator = !!process.env.FIREBASE_STORAGE_EMULATOR_HOST;
    console.log(
      `[data.ts] 1. listUserDocuments: Starte für ID ${companyId}. Verbinde mit ${
        isEmulator ? `EMULATOR (${process.env.FIREBASE_STORAGE_EMULATOR_HOST})` : 'LIVE Storage'
      }`
    );
    ...
    console.log(
      `[data.ts] DIAGNOSTIC: Found ${allFiles.length} total files in bucket. Files:`,
...
export async function getCompanyDetails(companyId: string): Promise<CompanyDetailData | null> {
  try {
    ...
    console.log(
      `[data.ts] B. getCompanyDetails: Parallele Abfragen (User, Company, Docs) abgeschlossen.`
    );

    // Wenn weder User- noch Firmen-Dokument existiert, können wir nichts anzeigen.
    if (!userDoc.exists && !companyDoc.exists) {
      console.warn(
        `[data.ts] C. getCompanyDetails: Weder User noch Company mit ID ${companyId} in Firestore gefunden.`
      );
      return null;
    }
    console.log(
      `[data.ts] C. getCompanyDetails: Mindestens ein Dokument (User oder Company) gefunden.`
    );
    ...
    const combinedDataRaw: { [key: string]: any } = {
      ...
      ...companyData,
      id: companyId,
      documents: documentsResult.files,
    };
    ...
    console.log(
      `[data.ts] E. getCompanyDetails: Serialisierung abgeschlossen. Daten werden zurückgegeben.`
    );
    return serializableData as CompanyDetailData;
  } catch (error) {
    console.error(
      `[data.ts] Schwerwiegender Fehler in getCompanyDetails für ID ${companyId}:`,
      error
    );
    return null;
  }
