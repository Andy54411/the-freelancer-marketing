...
export default function StripeFileDownloader({ fileId, label }: StripeFileDownloaderProps) {
    ...
    const handleDownload = async () => {
        ...
        setError(null);
        try {
            const getStripeFileLink = httpsCallable<{ fileId: string }, { url: string }>(functions, 'getStripeFileLink');
    ...
    return (
        <div className="flex flex-col items-end">
            <Button onClick={handleDownload} disabled={isLoading} variant="outline" size="sm">
                {isLoading ? <FiLoader className="animate-spin mr-2 h-4 w-4" /> : <FiDownload className="mr-2 h-4 w-4" />}
                {label}
            </Button>
