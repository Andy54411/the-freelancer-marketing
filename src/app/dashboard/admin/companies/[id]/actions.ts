...
export async function deleteCompany(companyId: string) {
  ...
  try {
    ...
    try {
      ...
      console.log(
        `[Action] Auth-Benutzer ${companyId} wurde nicht gefunden, was in diesem Fall in Ordnung ist.`
      );
    }

    revalidatePath('/dashboard/admin/companies');
    console.log(
      `[Action] Pfad /dashboard/admin/companies revalidiert. Löschvorgang für ${companyId} abgeschlossen.`
    );
    return {
      success: true,
      message: 'Firma und alle zugehörigen Daten wurden endgültig gelöscht.',
    };
  } catch (error: any) {
    ...
    if (error.code === 'auth/user-not-found') {
      errorMessage =
        'Der Authentifizierungs-Benutzer wurde nicht gefunden, die Datenbankeinträge wurden aber möglicherweise bereits gelöscht.';
