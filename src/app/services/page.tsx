      ...
      <div className="bg-white rounded-lg shadow-sm border border-gray-200 hover:shadow-md transition-shadow duration-200 cursor-pointer h-full">
        <div className="p-6">
          ...
          <div className="flex items-center justify-between mt-4 pt-4 border-t border-gray-100">
            <span className="text-sm text-[#14ad9f] font-medium">Anbieter durchsuchen</span>
  ...
  useEffect(() => {
    const fetchCategories = async () => {
      try {
        ...
        const providersQuery = query(collection(db, 'users'), where('isFreelancer', '==', true));

        const companiesQuery = query(collection(db, 'firma'), where('isActive', '==', true));
        ...
        const categoriesWithCounts = SERVICE_CATEGORIES.map(category => ({
          ...category,
          providerCount:
            Math.floor(totalCount / SERVICE_CATEGORIES.length) + Math.floor(Math.random() * 5),
        }));
      ...
      <div className="max-w-7xl mx-auto px-4">
        ...
        {filteredCategories.length === 0 ? (
          <div className="text-center py-20">
            ...
            <h3 className="text-xl font-semibold text-gray-900 mb-2">Keine Services gefunden</h3>
            <p className="text-gray-600 mb-6">
              {searchTerm
                ? `Keine Services für "${searchTerm}" gefunden.`
            ...
            {searchTerm && (
              <div className="mb-6">
                <p className="text-gray-600">
                  ...
                  für &ldquo;{searchTerm}&rdquo; gefunden
                </p>
              </div>
            )}
