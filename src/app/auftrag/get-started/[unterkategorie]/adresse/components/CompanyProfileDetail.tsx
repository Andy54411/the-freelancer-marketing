...
const CompanyProfileDetail: React.FC<CompanyProfileDetailProps> = ({
  ...
  return (
    <div className="fixed inset-0 z-50 bg-black/50 flex justify-center items-center px-4 py-4">
      <div className="bg-white rounded-lg shadow-lg max-w-2xl w-full max-h-[90vh] overflow-y-auto relative">
        <div className="sticky top-0 bg-white z-10 flex justify-end p-4 pb-2">
          <button
            onClick={onClose}
            className="text-gray-500 hover:text-gray-700 text-2xl leading-none bg-white rounded-full w-8 h-8 flex items-center justify-center hover:bg-gray-100"
          >
            &times;
          </button>
        </div>
        ...
        <div className="px-6 pb-6 -mt-2">
          <div className="flex flex-col sm:flex-row gap-4 items-center sm:items-start">
            ...
            <div className="flex-1 text-center sm:text-left">
              ...
              {/* Skills/Fähigkeiten als Badges */}
              {company.skills && Array.isArray(company.skills) && company.skills.length > 0 && (
                <div className="mt-2 mb-2">
                  <h3 className="text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">
                    Fähigkeiten:
                  </h3>
                  <div className="flex flex-wrap gap-2">
                    {company.skills.map((skill: string, index: number) => (
                      <span
                        key={index}
                        className="inline-block bg-[#14ad9f] text-white text-xs px-3 py-1 rounded-full font-medium"
                      >
                        {skill}
                      </span>
                    ))}
