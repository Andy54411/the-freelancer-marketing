...
import { useGoogleMapsApiLoader } from '@/components/GoogleMapsApiLoaderContext';
...
export default function AddressPage() {
  ...
  const { isMapsLoaded, mapsLoadError } = useGoogleMapsApiLoader(); // mapsLoadError wird jetzt verwendet
  ...
  useEffect(() => {
    // Wenn ein Maps-Ladefehler vorliegt, setzen Sie den allgemeinen Fehler
    if (mapsLoadError) {
      setError(`Kartenfehler: ${mapsLoadError.message}`);
    } else {
      // Löschen Sie den Kartenfehler, wenn er behoben ist, aber nicht andere Fehler
      if (error?.startsWith('Kartenfehler:')) {
        setError(null);
      }
    }
  ...
  }, [pathParams, registration, mapsLoadError, error]); // mapsLoadError als Dependency hinzugefügt
  ...
  const fetchDataForSubcategory = useCallback(async (subcategory: string) => {
    ...
    try {
      ...
      if (!res.ok) {
        const errorText = await res.text(); console.error(`${PAGE_ERROR} API getDataForSubcategory FAILED: ${res.status} ${res.statusText}. Response: ${errorText}`);
  ...
  useEffect(() => {
    if (selectedSubcategory) { fetchDataForSubcategory(selectedSubcategory); }
      ...
      setDynamicSliderMin(GLOBAL_FALLBACK_MIN_PRICE); setDynamicSliderMax(GLOBAL_FALLBACK_MAX_PRICE);
      setCurrentMaxPrice(GLOBAL_FALLBACK_MAX_PRICE); setAveragePriceForSubcategory(null);
      setPriceDistribution(null);
    }
  ...
  const fetchCompanyProfiles = useCallback(async () => {
    ...
    try {
      ...
      console.log('[AddressPage] Fetching company profiles with URL:', apiUrl);

      const res = await fetch(apiUrl);
      if (!res.ok) {
        const errorText = await res.text(); console.error(`${PAGE_ERROR} API searchCompanyProfiles FAILED: ${res.status} ${res.statusText}. Response: ${errorText}`);
  ...
  useEffect(() => {
    const timer = setTimeout(() => {
      if (postalCode && selectedSubcategory) fetchCompanyProfiles();
      else setCompanyProfiles([]);
    }, 500);
    return () => clearTimeout(timer);
  }, [fetchCompanyProfiles, postalCode, selectedSubcategory]);

  const handleAutocompleteLoad = (autocompleteInstance: google.maps.places.Autocomplete | null) => {
    if (autocompleteInstance) {
      autocompleteInstance.addListener('place_changed', () => {
        ...
        setStreetState(fullStreet.trim());
        setCityState(foundCity);
        if (foundPostalCode) setPostalCodeState(foundPostalCode);
        if (foundCountry) setCountryState(foundCountry);

        console.log(PAGE_LOG, "Autocomplete AddressPage Werte gesetzt:", { street: fullStreet.trim(), city: foundCity, postalCode: foundPostalCode, country: foundCountry });
      });
    }
  };
  ...
  const handleDateTimeConfirm: DateTimeSelectionPopupProps['onConfirm'] =
    async (selection?: Date | DateRange, time?: string, durationStringFromPopup?: string) => {
      ...
      if (selection) {
        if (selection instanceof Date && isValid(selection)) {
          ...
          if (effectiveTo < effectiveFrom) {
            ...
            setSelectedCompanyForPopup(null); return;
          }
      ...
      if (selectedCompanyForPopup && dateFromFormatted) {
        const anbieterStundensatzNum = typeof selectedCompanyForPopup.hourlyRate === 'number'
        ...
        console.log(PAGE_LOG, "Alle Auftragsdetails im Context aktualisiert für Navigation.");
