...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    const allInvoicesSnapshot = await db.collection('invoices')
      .where('companyId', '==', companyId)
      .orderBy('createdAt', 'desc')
      .get();

    console.log(`📊 Debug: Found ${allInvoicesSnapshot.size} total invoices for companyId: ${companyId}`);
    ...
    allInvoicesSnapshot.forEach((doc) => {
      ...
      // Erweitere Filter: Akzeptiere 'finalized', 'paid', oder 'sent' Status
      if (data.status === 'finalized' || data.status === 'paid' || data.status === 'sent') {
        invoices.push({
          id: doc.id,
          invoiceNumber: data.invoiceNumber || data.number,
    ...
    console.log(`✅ Filtered ${invoices.length} eligible invoices (finalized/paid/sent) from ${allInvoicesSnapshot.size} total`);
    ...
    console.log(`✅ Loaded ${filteredInvoices.length} invoices for reconciliation (status: ${status})`);
...
export async function POST(request: NextRequest) {
  try {
    ...
    console.log('🔄 Invoice reconciliation action:', { invoiceId, transactionId, companyId, action });
