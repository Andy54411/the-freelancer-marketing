...
export async function GET(request: NextRequest) {
  try {
    ...
    if (error) {
      ...
      return NextResponse.redirect(`${process.env.NEXT_PUBLIC_APP_URL}/dashboard/company/${state}/email-integration?error=gmail_auth_failed`);
    }

    if (!code || !state) {
      console.error('❌ Missing code or state in callback');
      return NextResponse.redirect(`${process.env.NEXT_PUBLIC_APP_URL}/dashboard/company/${state || 'unknown'}/email-integration?error=invalid_callback`);
    }
    ...
    await db.collection('companies').doc(state).collection('emailConfigs').doc(emailConfig.id).set(emailConfig);
    ...
    try {
      ...
      try {
        ...
        throw new Error(`Pub/Sub Topic Problem: ${topicError instanceof Error ? topicError.message : String(topicError)}`);
      }
      ...
      if (!watchResponse.data.historyId || !watchResponse.data.expiration) {
        throw new Error(`Incomplete watch response: historyId=${watchResponse.data.historyId}, expiration=${watchResponse.data.expiration}`);
      }
      ...
      await db.collection('companies').doc(state).collection('gmail_sync_status').doc(userInfo.data.email!).set(watchData);
      console.log('✅ Gmail Watch Status gespeichert in Subcollection');

      // DOPPELTE Verifikation: Prüfe ob gespeichert wurde
      const verifyDoc = await db.collection('companies').doc(state).collection('gmail_sync_status').doc(userInfo.data.email!).get();
      if (!verifyDoc.exists) {
      ...
      await db.collection('companies').doc(state).collection('emailConfigs').doc(emailConfig.id).update({
        watchEnabled: true,
      ...
      const errorWatchData = {
        ...
        errorDetails: watchError instanceof Error ? {
      ...
      try {
        await db.collection('companies').doc(state).collection('gmail_sync_status').doc(userInfo.data.email!).set(errorWatchData);
    ...
    try {
      ...
      if (response.ok) {
        console.log('✅ Initiale E-Mails werden geladen');
      } else {
        console.error('⚠️ Fehler beim initialen E-Mail-Sync');
      }
    } catch (syncError) {
      console.error('⚠️ Fehler beim initialen E-Mail-Sync:', syncError);
    }
