...
if (!admin.apps.length) {
    try {
        ...
        if (process.env.VERCEL) {
            console.log('[Firebase Server] Vercel-Umgebung erkannt. Initialisiere mit FIREBASE_SERVICE_ACCOUNT_KEY.');
            if (!serviceAccountKey) {
                console.error('[Firebase Server] KRITISCH: Die App läuft auf Vercel, aber die Umgebungsvariable FIREBASE_SERVICE_ACCOUNT_KEY ist nicht gesetzt.');
            ...
            try {
                ...
                console.error('[Firebase Server] KRITISCH: Parsen von FIREBASE_SERVICE_ACCOUNT_KEY fehlgeschlagen. Ist die Variable korrekt als JSON formatiert?', e);
            ...
            console.log('[Firebase Server] Lokale Umgebung erkannt. Initialisiere mit Application Default Credentials (erwartet GOOGLE_APPLICATION_CREDENTIALS).');
        ...
        if (process.env.NODE_ENV === 'development') {
            if (process.env.FIRESTORE_EMULATOR_HOST) console.log(`\x1b[32m%s\x1b[0m`, `[Firebase Server] Verbinde mit Firestore Emulator: ${process.env.FIRESTORE_EMULATOR_HOST}`);
            if (process.env.FIREBASE_AUTH_EMULATOR_HOST) console.log(`\x1b[32m%s\x1b[0m`, `[Firebase Server] Verbinde mit Auth Emulator: ${process.env.FIREBASE_AUTH_EMULATOR_HOST}`);
            if (process.env.FIREBASE_STORAGE_EMULATOR_HOST) console.log(`\x1b[32m%s\x1b[0m`, `[Firebase Server] Storage Emulator erkannt und wird verwendet: ${process.env.FIREBASE_STORAGE_EMULATOR_HOST}`);
        }
    } catch (error: any) {
