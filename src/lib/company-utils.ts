...
export function processCompanyData(companyData: CompanyData): ProcessedCompanyData {
    const displayName = (companyData.companyName as string || companyData.firmenname as string || `${companyData.firstName || ''} ${companyData.lastName || ''}`.trim()) || 'Unbenanntes Unternehmen';
    const status = (companyData.status === 'active' || companyData.status === 'locked') ? companyData.status : 'unknown';
    ...
    const displayedFields = new Set(['id', 'companyName', 'firmenname', 'firstName', 'lastName', 'email', 'status', ...foundDocumentKeys]);
    const otherData = Object.fromEntries(Object.entries(companyData).filter(([key]) => !displayedFields.has(key)));
