...
function calculateRealCompletion(companyData: ExistingCompanyUser): OnboardingProgress {
  ...
  return {
    ...
    completedAt: completionPercentage >= 80 ? companyData.createdAt : undefined,
    approvedAt: completionPercentage >= 80 ? companyData.createdAt : undefined,
    approvedBy: completionPercentage >= 80 ? 'system_migration' : undefined,
 ...
 */
export async function migrateLegacyCompanies(): Promise<{ success: number; errors: number; total: number }> {
  ...
  try {
    ...
    for (const companyDoc of companiesSnapshot.docs) {
      try {
        ...
        await setDoc(
          doc(db, 'users', companyDoc.id, 'onboarding', 'progress'), 
          onboardingProgress
        );
