          ...
          <table className="w-full bg-white relative">
            ...
            <tbody>
              {data.realItems.map((item, index) => {
                ...
                return (
                  <tr key={index} className={`${isEven ? 'bg-gray-50' : 'bg-white'} border-b border-gray-200`} style={PDF_PAGE_STYLES.tableRow}>
        ...
        <table className="w-full">
          <thead style={PDF_PAGE_STYLES.tableHeader}>
            <tr className="bg-gray-900 text-white">
              ...
              {hasAnyDiscount && (
                <th className="py-3 px-2 text-right font-mono text-xs">DISC</th>
              )}
          ...
          <tbody>
            {data.realItems.map((item, index) => {
              ...
              return (
                <tr key={index} className="border-b border-gray-300" style={PDF_PAGE_STYLES.tableRow}>
                  <td className="py-2 px-2 text-xs font-mono">{String(index + 1).padStart(3, '0')}</td>
                  ...
                  <td className="py-2 px-2 text-xs text-right font-mono">{formatCurrency(item.unitPrice)}</td>
                  {hasAnyDiscount && (
                    <td className="py-2 px-2 text-xs text-right font-mono text-red-600">
                      {discountPercent > 0 ? `-${discountPercent}%` : 'N/A'}
                    </td>
                  )}
                  <td className="py-2 px-2 text-xs text-right font-mono font-bold">{formatCurrency(discountedTotal)}</td>
                </tr>
              );
            })}
          </tbody>
