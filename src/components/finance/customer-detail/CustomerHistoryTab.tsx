...
export function CustomerHistoryTab({ customer }: CustomerHistoryTabProps) {
  ...
  useEffect(() => {
    ...
    const documentsUnsubscribe = onSnapshot(documentsRef, snapshot => {
    ...
    // WICHTIG: Erst ALLE Events laden, dann filtern
    // Weil wir nicht sicher sind, welches Feld die customerId speichert
    ...
    const calendarUnsubscribe = onSnapshot(
      ...
      snapshot => {
        ...
        snapshot.forEach(doc => {
          const data = doc.data();
          // Client-side filter: Prüfe verschiedene mögliche Felder
          if (
            data.customerId === customer.id ||
            data.kundenId === customer.id ||
            data.contactId === customer.id ||
            data.customerKundenNummer === customer.kundenNummer ||
            data.kundenNummer === customer.kundenNummer
          ) {
            loadedEvents.push({
              id: doc.id,
              ...data,
    ...
    // Set loading to false after a timeout if no data comes in
  ...
  useEffect(() => {
    updateStats(activities, calendarEvents, documents);
  }, [activities, calendarEvents, documents]);

  // Update stats function (called directly from onSnapshot callbacks)
  const updateStats = (
    ...
    // Count ALL calendar events as meetings (not just specific types)
    // Da wir alle Termine zählen wollen, unabhängig vom eventType
    const calendarMeetings = eventList.length;

    // Count ONLY real documents from documents collection
    // Activities vom Typ 'document' sind nur Log-Einträge, keine echten Dokumente
    const realDocuments = docList.length;

    const newStats = {
      calls: activityCalls,
      emails: activityEmails,
      meetings: activityMeetings + calendarMeetings,
      documents: realDocuments, // NUR echte Dokumente aus der documents Collection
    };

    setStats(newStats);

    // IMMER loading auf false setzen wenn updateStats aufgerufen wird
    // Das bedeutet, dass Daten geladen wurden (auch wenn leer)
    setLoading(false);
  };
    ...
    if (metadata?.quoteId || metadata?.quoteNumber) {
      switch (metadata?.actionType) {
        ...
        case 'deleted':
          return 'bg-gray-500';
        default:
          return 'bg-indigo-500'; // Default für Angebote
      }
    }
    ...
    if (metadata?.invoiceId || metadata?.invoiceNumber) {
      switch (metadata?.actionType) {
        ...
        case 'storno':
          return 'bg-red-600';
        default:
          return 'bg-emerald-500'; // Default für Rechnungen
      }
    }
  ...
  const getCombinedTimeline = () => {
    ...
    calendarEvents.forEach(event => {
      ...
      combinedItems.push({
        ...
        description:
          event.description ||
          `${event.eventType} am ${new Date(event.startDate).toLocaleDateString('de-DE')} um ${event.startTime}`,
        timestamp: eventDate,
        user: 'Kalendersystem',
  ...
  return (
    <div className="space-y-6 h-full">
      <Tabs defaultValue="activities" className="w-full h-full flex flex-col">
        ...
        <TabsContent value="activities" className="mt-6 flex-1 overflow-auto">
          <div className="space-y-6 h-full">
            {/* Statistics Cards */}
            <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
              <Card
                className="cursor-pointer hover:shadow-md hover:border-[#14ad9f] transition-all duration-200"
              ...
              <Card
                className="cursor-pointer hover:shadow-md hover:border-[#14ad9f] transition-all duration-200"
                onClick={() => toast.info('E-Mail-Funktion kommt bald')}
              >
                <CardContent className="p-4">
                  <div className="flex items-center gap-2">
              ...
              <Card
                ...
                onClick={() => {
                  ...
                  ).find(tab => tab.textContent?.includes('Kalender')) as HTMLElement;
                  calendarTab?.click();
                }}
              >
                <CardContent className="p-4">
              ...
              <Card
                ...
                onClick={() => {
                  ...
                  const documentsTab = Array.from(
                    tabsList?.querySelectorAll('[role="tab"]') || []
                  ).find(tab => tab.textContent?.includes('Workspace')) as HTMLElement;
                  documentsTab?.click();
                }}
              >
                <CardContent className="p-4">
                    ...
                    return combinedTimeline.length === 0 ? (
                      <div className="text-center py-8 text-gray-500">
                        ...
                        <p className="text-sm mb-4">
                          Fügen Sie die erste Notiz oder den ersten Anruf hinzu
                        </p>
                      </div>
                    ) : (
                      <div
                        ...
                        <div className="space-y-6 pb-4">
                          {combinedTimeline.map((item, index) => (
                            ...
                            >
                              {/* Timeline dot */}
                              <div
                                className={`relative z-10 flex items-center justify-center w-12 h-12 rounded-full ${getActivityColor(item.type, item.metadata)} text-white`}
                              >
                              ...
                              <div className="flex-1 pb-6">
                                <div
                                  className={`rounded-lg p-4 ${item.source === 'calendar' ? 'bg-blue-50 border border-blue-200' : 'bg-gray-50'}`}
                                >
