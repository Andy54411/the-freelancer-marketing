...
interface EmailSendModalProps {
  ...
  // User data for template rendering
  userData?: {
    firstName?: string;
    lastName?: string;
    email?: string;
  ...
  // Optional callback
  onSend?: (method: 'email', options?: any) => Promise<void>;
  companyData?: {
    name: string;
    email: string;
    signature?: string;
...
export function EmailSendModalNormal({
  ...
  documentType,
  companyId,
  selectedLayout = 'TEMPLATE_NEUTRAL',
  selectedColor = '#14ad9f',
  logoUrl,
  logoSize = 50,
  pageMode = 'single',
  documentSettings = {},
  ...
  // ✅ Generate PDF using LivePreviewModal rendering logic (same as LivePreviewModal.tsx)
...
<body>
  ...
  // Helper function to send HTML to PDF API
  const sendToApi = async (htmlContent: string): Promise<string> => {
  ...
  const handleSend = async () => {
    ...
    // Direct send - no GoBD logic in normal modal
    await performSend();
  };
