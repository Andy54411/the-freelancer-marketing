...
export function SendDocumentModal({
  ...
  useEffect(() => {
    ...
    console.log('🚀 DOCUMENT LOADED - Setting initial pageMode:', {
      newItemsCount,
      newSmartMode,
  ...
  useEffect(() => {
    ...
    console.log('🤖 SMART PAGE MODE CHECK:', {
      newItemsCount,
      currentPageMode: pageMode,
      recommendedMode: newSmartMode,
      manualOverride,
    });
  ...
  useEffect(() => {
    console.log('🔄 PageMode changed to:', pageMode, '- Clearing cached HTML');
    setCachedHtml(null);
  ...
  useEffect(() => {
    console.log('🔥 FORCING HTML CACHE CLEAR');
    setCachedHtml(null);
    setPreExtractedHtml(null);
  ...
  const handlePageModeChange = (newPageMode: 'single' | 'multi') => {
    console.log('👤 MANUAL pageMode change to:', newPageMode);
  ...
  useEffect(() => {
    if (isOpen) {
      ...
      console.log('Template readiness check:', {
        ...
        hasInnerHTML: !!templateRef.current?.innerHTML,
        hasData,
        hasTemplateElement,
        ready,
      });
  ...
  const cacheTemplateHtml = useCallback(async () => {
    ...
    try {
      ...
      console.log('✅ HTML cached successfully, length:', htmlContent?.length || 0);
    } catch (error) {
  ...
  const extractHtmlWithStyles = useCallback(async () => {
    ...
    console.log('🎨 Extracting HTML with ALL styles...');
    ...
    styleTags.forEach((styleElement, index) => {
      if (styleElement.textContent) {
        console.log(`📋 Style tag ${index + 1} length:`, styleElement.textContent.length);
    ...
    console.log('🔗 Found', linkTags.length, 'linked stylesheets');
    ...
    linkTags.forEach((linkElement, index) => {
      try {
        ...
        if (link.sheet && link.sheet.cssRules) {
          ...
          console.log(`🔗 Stylesheet ${index + 1} extracted CSS length:`, css.length);
          allStyles.push(css);
        }
      } catch (e) {
        console.log(`🔗 Stylesheet ${index + 1} - Cross-origin, skipped`);
      }
    });
    ...
    console.log(
      ...
      allStyles.length,
      'sources, total CSS length:',
      allStyles.join('').length
    );
...
<body>
  ${clonedElement.outerHTML}
    ...
    console.log('✅ Complete HTML with styles created, length:', completeHtml.length);
    return completeHtml;
  }, []);
...
${document.companyName || 'Ihr Unternehmen'}`;
  ...
  const isTemplateReady = useCallback(() => {
    ...
    console.log('isTemplateReady called:', {
      templateReady,
      basicReady,
      loadingEInvoiceData,
      hasRealDocumentData: !!realDocumentData,
      hasDocument: !!document,
      finalResult: basicReady,
    });
  ...
  const getRenderedHtml = useCallback(async () => {
    ...
    if (preExtractedHtml) {
      console.log('✅ Using pre-extracted HTML, length:', preExtractedHtml.length);
      return preExtractedHtml;
    }

    // Second try to use cached HTML
    if (cachedHtml) {
      console.log('✅ Using cached HTML, length:', cachedHtml.length);
      return cachedHtml;
    }

    // Last resort: try to extract immediately
    console.log('🔄 Extracting HTML from current template as last resort...');
    ...
    <Dialog open={isOpen} onOpenChange={() => {}}>
                ...
                <div className="border-b">
                  <div
                    ...
                    onClick={async () => {
                      ...
                      console.log('🚀 Pre-extracting HTML before opening EmailSendModal...');
                      try {
                        const html = await extractHtmlWithStyles();
                        setPreExtractedHtml(html);
                        console.log(
                          '✅ HTML pre-extracted successfully, length:',
                          html?.length || 0
                        );
                      } catch (error) {
      ...
      <EmailSendModal
        ...
        onSend={async emailData => {
          console.log(
            'SendDocumentModal - Selected Layout passed to EmailSendModal:',
            selectedLayout
          );
          if (onSend) {
