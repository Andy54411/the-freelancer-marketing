...
} from '@/data/datev-income-mapping-complete';
    ...
    if (expenseGroup) {
      ...
      expenseCards.forEach(card => {
        ...
        if (mapping) {
          ...
          console.log(`✅ Kategorisiert: ${card.code} (${card.name}) → ${categoryId}`);
        } else {
      ...
      Object.entries(categorizedCards).forEach(([categoryId, cards]) => {
        ...
        if (targetCategory) {
          ...
          if (categoryId === 'banken-finanzen' || categoryId === 'sonstiges') {
            console.log(`🎯 Kategorie ${categoryId}: ${cards.length} DATEV-Konten zugewiesen`);
            cards.forEach(card => console.log(`  - ${card.code}: ${card.name}`));
          }
        }
      });
    }
    ...
    if (incomeGroup) {
      ...
      incomeCards.forEach(card => {
        ...
        if (incomeMapping) {
          ...
          console.log(`✅ INCOME kategorisiert: ${card.code} (${card.name}) → ${categoryId}`);
        } else {
          ...
          console.log(`⚠️ INCOME unbekannt: ${card.code} (${card.name}) → sonstige-erträge`);
        }
      });
      ...
      Object.entries(categorizedIncomeCards).forEach(([categoryId, cards]) => {
        ...
        if (targetCategory) {
          ...
          console.log(`🎯 INCOME-Kategorie ${categoryId}: ${cards.length} DATEV-Konten zugewiesen`);
          cards.forEach(card => console.log(`  - ${card.code}: ${card.name}`));
        }
      });
    }
