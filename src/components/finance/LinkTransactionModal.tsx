...
export default function LinkTransactionModal({ 
  ...
  const loadInvoices = async () => {
    ...
    try {
      ...
      const convertedDocuments: Document[] = invoices.map((invoice, index) => ({
        ...
        type: invoice.isStorno ? 'gutschrift' : 'rechnung',
        number: invoice.documentNumber || invoice.invoiceNumber || invoice.id || `DOC-${index}`,
  ...
  const handleCreateDocument = () => {
    ...
    console.log('Create new document of type:', activeTab);
  };
