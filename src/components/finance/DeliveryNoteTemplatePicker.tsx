...
interface DeliveryNoteTemplatePickerProps {
  ...
  onTemplateSelect?: (template: DeliveryNoteTemplate) => void;
  selectedTemplate?: DeliveryNoteTemplate;
  userId?: string;
}
...
export function DeliveryNoteTemplatePicker({
  ...
  const [previewTemplate, setPreviewTemplate] = useState<DeliveryNoteTemplate | null>(null);
  const [saving, setSaving] = useState(false);

  const handleTemplateSelect = async (template: DeliveryNoteTemplate) => {
    try {
      setSaving(true);
      ...
      setOpen(false);
      toast.success(
        `Lieferschein-Template "${(AVAILABLE_DELIVERY_NOTE_TEMPLATES as readonly any[]).find((t: any) => t.id === template)?.name}" ausgewählt`
      );
    } catch (_error) {
  ...
  const handlePreview = (template: DeliveryNoteTemplate) => {
    ...
    <>
      <Dialog open={open} onOpenChange={setOpen}>
        <DialogTrigger asChild>
          {trigger || <Button variant="outline">Lieferschein-Template auswählen</Button>}
        </DialogTrigger>
        <DialogContent className="sm:max-w-[1000px] max-h-[80vh] overflow-y-auto">
          <DialogHeader>
            <DialogTitle>Lieferschein-Template auswählen</DialogTitle>
            <DialogDescription>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 mt-6">
            {(AVAILABLE_DELIVERY_NOTE_TEMPLATES as readonly any[]).map((template: any) => (
          ...
          <DialogContent className="sm:max-w-[800px] max-h-[80vh]">
            <DialogHeader>
              <DialogTitle>
                Vorschau:{' '}
                {
                  (AVAILABLE_DELIVERY_NOTE_TEMPLATES as readonly any[]).find(
