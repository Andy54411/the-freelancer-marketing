...
export default function OnboardingStep5() {
  ...
  const handleSubmit = async () => {
    if (!step5Data.documentsCompleted) return;
    
    setIsSubmitting(true);
    try {
      await submitOnboarding();
      // Navigation wird vom OnboardingContext nach erfolgreichem Submit übernommen
    } catch (error) {
  ...
  const isReadyToSubmit = step5Data.documentsCompleted && completionPercentage > 80;
    ...
    <div className="max-w-4xl mx-auto p-6 space-y-8">
              ...
              <div className="border-l-4 border-[#14ad9f] pl-4">
                ...
                <div className="text-sm text-gray-600 space-y-1">
                  {stepData[1].businessType && (
                    <p>Unternehmenstyp: <span className="font-medium">{stepData[1].businessType}</span></p>
                  )}
                  {stepData[1].employees && (
                    <p>Mitarbeiteranzahl: <span className="font-medium">{stepData[1].employees}</span></p>
                  )}
                  {stepData[1].website && (
                    <p>Website: <span className="font-medium">{stepData[1].website}</span></p>
                  )}
                </div>
              </div>
            )}
              ...
              <div className="border-l-4 border-[#14ad9f] pl-4">
                ...
                <div className="text-sm text-gray-600 space-y-1">
                  {stepData[2].kleinunternehmer && (
                    <p>Kleinunternehmerregelung: <span className="font-medium">
                      {stepData[2].kleinunternehmer === 'ja' ? 'Ja' : 'Nein'}
                    </span></p>
                  )}
                  {stepData[2].profitMethod && (
                    <p>Gewinnermittlung: <span className="font-medium">
                      {stepData[2].profitMethod === 'euer' ? 'EÜR' : 'Bilanz'}
                    </span></p>
                  )}
                  {stepData[2].priceInput && (
                    <p>Preiseingabe: <span className="font-medium">
              ...
              <div className="border-l-4 border-[#14ad9f] pl-4">
                ...
                <div className="text-sm text-gray-600 space-y-1">
                  {stepData[3].skills && stepData[3].skills.length > 0 && (
                    <p>Fähigkeiten: <span className="font-medium">{stepData[3].skills.length} definiert</span></p>
                  )}
                  {stepData[3].servicePackages && stepData[3].servicePackages.length > 0 && (
                    <p>Service-Pakete: <span className="font-medium">{stepData[3].servicePackages.length} erstellt</span></p>
                  )}
                  {stepData[3].instantBooking && (
                    <p>Sofortbuchung: <span className="font-medium">Aktiviert</span></p>
                  )}
                </div>
              </div>
            )}
              ...
              <div className="border-l-4 border-[#14ad9f] pl-4">
                ...
                <div className="text-sm text-gray-600 space-y-1">
                  {stepData[4].availabilityType && (
                    <p>Verfügbarkeit: <span className="font-medium">
                  ...
                  {stepData[4].maxTravelDistance && (
                    <p>Max. Entfernung: <span className="font-medium">{stepData[4].maxTravelDistance} km</span></p>
                  )}
                  {stepData[4].serviceAreas && stepData[4].serviceAreas.length > 0 && (
                    <p>Service-Gebiete: <span className="font-medium">{stepData[4].serviceAreas.length} definiert</span></p>
                  )}
                </div>
              </div>
            )}
              ...
              <div className="flex items-start gap-3">
                ...
                <div>
                  ...
                  <div className="flex items-center space-x-2">
                    ...
                    />
                    <Label htmlFor="documentsCompleted" className="text-sm font-medium text-blue-900">
        ...
        <Card className="bg-gray-50">
          <CardContent className="pt-6">
            ...
            <div className="space-y-3 text-sm text-gray-600">
              <div className="flex items-start gap-3">
                <div className="w-6 h-6 bg-[#14ad9f] text-white rounded-full flex items-center justify-center text-xs font-medium">1</div>
                <div>
              ...
              <div className="flex items-start gap-3">
                <div className="w-6 h-6 bg-[#14ad9f] text-white rounded-full flex items-center justify-center text-xs font-medium">2</div>
                <div>
                  <p className="font-medium text-gray-900">Freischaltung</p>
                  <p>Bei erfolgreicher Prüfung wird Ihr Profil aktiviert und Sie erhalten eine E-Mail</p>
                </div>
              </div>
              <div className="flex items-start gap-3">
                <div className="w-6 h-6 bg-[#14ad9f] text-white rounded-full flex items-center justify-center text-xs font-medium">3</div>
                <div>
      ...
      <div className="flex justify-between pt-6">
        ...
        <Button 
          ...
          className="px-8 bg-[#14ad9f] hover:bg-[#129488] text-white disabled:bg-gray-300"
        >
          {isSubmitting ? (
