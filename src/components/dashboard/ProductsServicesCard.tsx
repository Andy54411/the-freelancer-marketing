...
export default function ProductsServicesCard({
  ...
  const loadProductsAndServices = async () => {
    ...
    try {
      ...
      console.log('🔄 ProductsServicesCard: Loading products for company:', activeCompanyId);
      
      // 🔧 AUTOMATISCHE REPARATUR: Fehlende inventoryItemIds reparieren
      console.log('🔧 AUTO-REPAIR: Triggering invoice repair workflow...');
      try {
        const { ServiceUsageTrackingService } = await import('@/services/serviceUsageTrackingService');
        
        // Alle Rechnungen laden und prüfen
        const invoicesForRepair = await getDocs(collection(db, 'companies', activeCompanyId, 'invoices'));
        console.log(`🔧 AUTO-REPAIR: Checking ${invoicesForRepair.size} invoices for repair needs...`);
        
        let repairedCount = 0;
        for (const invoiceDoc of invoicesForRepair.docs) {
          const wasRepaired = await ServiceUsageTrackingService.repairInvoiceInventoryIds(
            activeCompanyId, 
            invoiceDoc.id
          );
          if (wasRepaired) {
            repairedCount++;
            // Nach Reparatur das Tracking ausführen
            const invoiceData = invoiceDoc.data();
            await ServiceUsageTrackingService.trackInvoiceServiceUsage(
              activeCompanyId,
              invoiceDoc.id,
              invoiceData.items || [],
              invoiceData.customerName || 'Unbekannt'
            );
          }
        }
        
        if (repairedCount > 0) {
          console.log(`✅ AUTO-REPAIR: Successfully repaired and tracked ${repairedCount} invoices`);
        } else {
          console.log('✅ AUTO-REPAIR: No repairs needed');
        }
      } catch (repairError) {
        console.error('⚠️ AUTO-REPAIR: Error during repair workflow:', repairError);
        // Continue loading even if repair fails
      }
      ...
      console.log(`📦 Found ${servicePackagesSnapshot.size} service packages`);
      console.log(`🔧 Found ${inlineServicesSnapshot.size} inline services`);
      console.log(`📄 Found ${invoicesSnapshot.size} invoices for analysis`);
      ...
      console.log('✅ ProductsServicesCard: Loaded total', allProducts.length, 'products/services');
      ...
      if (invoice.items && Array.isArray(invoice.items)) {
        invoice.items.forEach((item: any) => {
          ...
          if (!item.servicePackageId && !item.inventoryItemId && item.name) {
            ...
            const matchingServicePackage = products.find(p => 
              p.type === 'servicePackage' && 
            ...
            const matchingInlineService = products.find(p => 
              p.type === 'inlineService' && 
              p.name.toLowerCase() === item.name.toLowerCase()
            );
      ...
      case 'servicePackage':
        return (
          <svg width="16" height="16" viewBox="0 0 24 24" fill="none" className="text-blue-600">
            <path d="M20 7L12 3L4 7L12 11L20 7Z" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round" />
      ...
      case 'inlineService':
        return (
          <svg width="16" height="16" viewBox="0 0 24 24" fill="none" className="text-purple-600">
            <path d="M12 2L2 7V10C2 16 6 20.88 12 22C18 20.88 22 16 22 10V7L12 2Z" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round" />
          </svg>
        );
    }
  };
        ...
        <div className="mb-4">
          ...
          {products.length > 0 && (
            <p className="text-sm text-gray-500 mt-1">
              {products.length} Services • {topProducts.length} mit Verkäufen • {noSalesProducts.length} ohne Verkäufe
            </p>
          )}
        </div>
                ...
                <div className="flex items-center gap-3 mb-6">
                  <span className="flex items-center justify-center w-10 h-10 bg-green-50 rounded-lg">
                    <svg width="24" height="24" viewBox="0 0 24 24" fill="none" className="text-green-600">
                      <path d="M17.5 6.5L6.5 17.5M8.7451 5.75H17.25C17.8023 5.75 18.25 6.19772 18.25 6.75V16.2458" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round" />
                    </svg>
                  </span>
                  <span className="text-lg font-semibold text-gray-900">Höchste Umsätze (Netto)</span>
                </div>
                <ul className="space-y-4">
                  {topProducts.map((product, index) => (
                    <li key={product.id} className="flex items-center justify-between py-4 px-4 bg-gray-50 rounded-lg hover:bg-gray-100 transition-colors">
                ...
                <div className="flex items-center gap-3 mb-6">
                  <span className="flex items-center justify-center w-10 h-10 bg-red-50 rounded-lg">
                    <svg width="24" height="24" viewBox="0 0 24 24" fill="none" className="text-red-600">
                      <path d="M17.5 17.5L6.5 6.5M18.25 8.7451V17.25C18.25 17.8023 17.8023 18.25 17.25 18.25H7.75419" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round" />
                    </svg>
                  </span>
                  <span className="text-lg font-semibold text-gray-900">Geringster Umsatz (Netto)</span>
                </div>
                <ul className="space-y-4">
              ...
              <div className="bg-white">
                ...
                <div className="grid grid-cols-1 gap-3">
                  {noSalesProducts.slice(0, 3).map((product, index) => (
                    <div key={product.id} className="flex items-center justify-between py-3 px-4 bg-gray-25 rounded-lg border border-gray-100">
                      <div className="flex items-center gap-3">
                        ...
                        <div className="flex flex-col">
                          ...
                          <div className="text-xs text-gray-500">
                            {product.type === 'servicePackage' ? 'Service-Paket' : `Inline-Service (${product.unit})`} • {formatAmount(product.price)}
                          </div>
                        </div>
                      </div>
      ...
      <div className="px-6 py-4 bg-gray-50 border-t border-gray-200 rounded-b-lg">
        <div className="flex items-center justify-between">
          {requiresData && products.length === 0 && (
            <div className="text-sm text-gray-500">Erstellen Sie Services um Verkaufsdaten zu sehen</div>
          )}
