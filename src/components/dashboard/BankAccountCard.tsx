...
export default function BankAccountCard({
  ...
  const loadRecentTransactions = React.useCallback(async () => {
    try {
      ...
      const response = await fetch(
        `/api/finapi/transactions-enhanced?userId=${user.uid}&accountIds=${selectedAccounts.join(',')}&credentialType=${credentialType}`
      );
                                ...
                                <div className="flex items-center gap-3">
                                  ...
                                  <div>
                                    ...
                                    <div className="text-xs text-gray-500">
                                      <span>{account.iban ? `${account.iban.slice(0, 8)}...` : account.accountNumber || ''}</span>
                                    </div>
                                  </div>
                                </div>
                                <div>
                                  <span className="text-sm font-semibold text-gray-900">{formatAmount(account.balance)}</span>
                                </div>
                              </div>
                              <div>
                              ...
                              <div>
                                ...
                                <div className="flex items-center text-xs text-gray-500 mt-0.5">
                                  <div className="flex items-center gap-1">
                                    ...
                                    <svg className="w-1 h-1 text-gray-400" width="3" height="3" viewBox="0 0 100 100" xmlns="http://www.w3.org/2000/svg">
                                      <circle cx="50" cy="50" r="50" fill="currentColor"></circle>
                                    </svg>
                                    <span className="truncate max-w-[120px]">{transaction.purpose}</span>
                                  </div>
                                </div>
                              </div>
                            </div>
