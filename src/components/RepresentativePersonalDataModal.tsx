...
const RepresentativePersonalDataModal: React.FC<RepresentativePersonalDataModalProps> = ({
  ...
  const {
    ...
    email,
    phoneNumber,
    dateOfBirth, // Aus Step 1
    personalStreet,
    setPersonalStreet,
    personalHouseNumber,
    setPersonalHouseNumber,
    personalPostalCode,
    ...
    setActualOwnershipPercentage,
    isActualExecutive,
    setIsActualExecutive,
    actualRepresentativeTitle,
    setActualRepresentativeTitle,
  } = useRegistration();
  ...
  const [localActualOwnershipPercentage, setLocalActualOwnershipPercentage] = useState(
    actualOwnershipPercentage?.toString() || ''
  );
  const [localIsActualExecutive, setLocalIsActualExecutive] = useState(isActualExecutive ?? false);
  const [localActualRepresentativeTitle, setLocalActualRepresentativeTitle] = useState(
    actualRepresentativeTitle || 'Gesetzlicher Vertreter'
  );
  ...
  useEffect(() => {
    if (isOpen) {
      ...
      const isRelevantCompanyType =
        currentLegalFormLower.includes('gmbh') ||
    ...
    actualOwnershipPercentage,
    isActualExecutive,
    actualRepresentativeTitle, // Dependencies für granulare Rollen
  ]);
  ...
  const handleSubmit = () => {
    ...
    if (!isManagingDirectorOwner) {
      if (localIsActualOwner) {
        ...
        if (isNaN(percentageNum) || percentageNum <= 0 || percentageNum > 100) {
          setFormError(
            "Bitte geben Sie einen gültigen Eigentumsanteil (1-100) an oder deaktivieren Sie 'Ist Eigentümer'."
          );
          return;
        }
      }
    ...
    if (isManagingDirectorOwner && isRelevantCompanyType) {
      ...
      if (setActualRepresentativeTitle)
        setActualRepresentativeTitle('Geschäftsführender Gesellschafter');
      ...
      if (setActualOwnershipPercentage) {
        const percentageNum = parseInt(localActualOwnershipPercentage, 10);
        setActualOwnershipPercentage(
          localIsActualOwner && !isNaN(percentageNum) ? percentageNum : undefined
        );
      }
      if (setIsActualExecutive) setIsActualExecutive(localIsActualExecutive);
      if (setActualRepresentativeTitle)
        setActualRepresentativeTitle(localActualRepresentativeTitle.trim());
      ...
      if (setActualRepresentativeTitle)
        setActualRepresentativeTitle(
          isManagingDirectorOwner ? 'Geschäftsführer und Inhaber' : 'Gesetzlicher Vertreter'
        );
    }
      ...
      <div className="bg-white dark:bg-gray-800 rounded-lg shadow-xl p-6 w-full max-w-lg max-h-[90vh] overflow-y-auto">
        ...
        <div className="space-y-3 mb-4 p-3 bg-gray-50 dark:bg-gray-700 rounded-md">
          <p className="text-sm text-gray-600 dark:text-gray-300">
            Folgende Daten wurden bereits erfasst:
          </p>
          <p>
          ...
          <p>
            <strong>Geburtsdatum:</strong> {dateOfBirth || 'N/A (Bitte in Step 1 erfassen)'}
          </p>
          {showOwnershipInfo && (
            <p className="text-sm text-teal-700 dark:text-teal-400 mt-2">
              <strong>Hinweis:</strong> Da Sie als alleiniger Geschäftsführer und Inhaber einer
              Kapitalgesellschaft (GmbH/UG/AG) agieren, werden Ihre Rollen entsprechend gesetzt und
              Ihr Eigentumsanteil auf 100%.
            </p>
          )}
        </div>
        ...
        <p className="text-sm text-gray-700 dark:text-gray-300 mb-1">
          Bitte vervollständigen Sie die Privatadresse:
        </p>
        <button
          type="button"
        ...
        <div className="space-y-3">
          ...
          <div>
            <label
              htmlFor="repPersonalHouseNumber"
              className="block text-sm font-medium text-gray-700 dark:text-gray-300"
            >
              Hausnummer
            </label>
          ...
          <div>
            <label
              htmlFor="repPersonalCountry"
              className="block text-sm font-medium text-gray-700 dark:text-gray-300"
            >
              Land (ISO-Code)*
            </label>
            <select
              id="repPersonalCountry"
            ...
            <div className="space-y-3">
              ...
              <label className="flex items-center">
                ...
                <span className="text-sm text-gray-700 dark:text-gray-300">
                  Ist offizieller Direktor / Geschäftsführer?
                </span>
              </label>
              <label className="flex items-center">
                <input
                  type="checkbox"
                  checked={localIsActualExecutive}
                  onChange={e => setLocalIsActualExecutive(e.target.checked)}
                  className="mr-2 h-4 w-4 text-teal-600 border-gray-300 rounded focus:ring-teal-500"
                />
                <span className="text-sm text-gray-700 dark:text-gray-300">
                  Ist leitende Führungskraft?
                </span>
              </label>
              <label className="flex items-center">
                <input
                  type="checkbox"
                  checked={localIsActualOwner}
                  onChange={e => setLocalIsActualOwner(e.target.checked)}
                  className="mr-2 h-4 w-4 text-teal-600 border-gray-300 rounded focus:ring-teal-500"
                />
                <span className="text-sm text-gray-700 dark:text-gray-300">
                  Ist wirtschaftlich Berechtigter / Eigentümer?
                </span>
              </label>
              {localIsActualOwner && (
                <div className="ml-6 mt-2">
                  <label
                    htmlFor="actualOwnershipPercentage"
                    className="block text-sm font-medium text-gray-700 dark:text-gray-300"
                  >
                    Eigentumsanteil in %*
                  </label>
