...
export function BookingDetailsDrawer({ auftrag, children }: BookingDetailsDrawerProps) {
    ...
    const calculateTotalHours = (currentAuftrag: Auftrag): string => {
        ...
        if (jobDateFrom && jobDateTo) {
            try {
                ...
                if (!isNaN(dateFromInstance.getTime()) && !isNaN(dateToInstance.getTime())) {
                    const dayStartFrom = new Date(dateFromInstance.getFullYear(), dateFromInstance.getMonth(), dateFromInstance.getDate());
                    const dayStartTo = new Date(dateToInstance.getFullYear(), dateToInstance.getMonth(), dateToInstance.getDate());

                    if (dayStartTo.getTime() < dayStartFrom.getTime()) {
                        return 'Enddatum liegt vor Startdatum';
                    }

                    const numberOfDays = Math.round((dayStartTo.getTime() - dayStartFrom.getTime()) / (1000 * 60 * 60 * 24)) + 1;
                    ...
                    if (parsedDailyOrTotalHours === null) { // Nur wenn jobDurationString nicht für tägliche Rate verwendet wurde
                        ...
                        if (diffMilliseconds >= 0) {
                            ...
                            if (diffHours > 0 || (diffHours === 0 && numberOfDays === 1 && dateFromInstance.getTime() === dateToInstance.getTime())) {
        ...
        if (parsedDailyOrTotalHours !== null) { // Aus jobDurationString
                ...
                <div className="flex flex-col gap-6 overflow-y-auto px-6 text-sm">
                    ...
                    <div className="mt-6 pt-6 border-t">
                        ...
                        <Button
                            variant="default"
                            className="w-full"
                            onClick={() => setIsChatModalOpen(true)}> {/* <-- Updated onClick */}
                            Nachricht an {auftrag.customerFirstName || 'Kunden'} senden
                        </Button>
                    </div> {/* Schließt den "Chat-Bereich" div */}
                </div> {/* Schließt den Haupt-Inhalts-Div "flex flex-col gap-6..." */}

                <DrawerFooter className="mt-auto p-6"> {/* DrawerFooter als direkter Child von DrawerContent */}
                    <DrawerClose asChild>
