...
export function ModernScheduleView({
  ...
  const getStatistics = () => {
    ...
    const timeStats = {
      EARLY: shifts.filter(shift => getShiftTypeByTime(shift.startTime, shift.status) === 'EARLY').length,
      MIDDLE: shifts.filter(shift => getShiftTypeByTime(shift.startTime, shift.status) === 'MIDDLE').length,
      LATE: shifts.filter(shift => getShiftTypeByTime(shift.startTime, shift.status) === 'LATE').length,
      NIGHT: shifts.filter(shift => getShiftTypeByTime(shift.startTime, shift.status) === 'NIGHT').length,
    };
  ...
  const CustomDayHeader = ({ date, label }: { date: Date; label: string }) => {
    ...
    return (
      <div className={cn(
        ...
        <div className={cn(
          'text-sm font-medium',
          today ? 'text-[#14ad9f]' : 'text-gray-700'
        )}>
          {label}
        </div>
          ...
          {selectedShift && (
            <>
              <DialogHeader>
                ...
                <DialogDescription>
                  Informationen zur ausgewählten Schicht
                </DialogDescription>
              </DialogHeader>
              ...
              <div className="space-y-4">
                {(() => {
                  ...
                  const shiftType = SHIFT_TYPES[selectedShift.status as keyof typeof SHIFT_TYPES] || SHIFT_TYPES.PLANNED;
                  const Icon = shiftType.icon;
                    ...
                    <>
                      ...
                      <div className="grid grid-cols-2 gap-4">
                        <div>
                          ...
                          <p className="font-medium">{moment(selectedShift.date).format('DD.MM.YYYY')}</p>
                        </div>
                        <div>
          ...
          {selectedShiftType && (
            <>
              <DialogHeader>
                ...
                <DialogDescription>
                  Übersicht aller Schichten in dieser Kategorie
                </DialogDescription>
              </DialogHeader>
