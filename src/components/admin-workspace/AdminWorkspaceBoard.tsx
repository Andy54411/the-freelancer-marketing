...
export function AdminWorkspaceBoard({
  ...
  const handleDragEnd = (result: DropResult) => {
    ...
    if (source.droppableId === destination.droppableId) {
      ...
      const updatedTasks = newTasks.map(
        (task, index) =>
          ({
            ...task,
            position: index,
            // Ensure all required AdminWorkspaceTask fields are present
            assignee: task.assignee || '',
            assignees: task.assignees || [],
            labels: task.labels || [],
            checklist: task.checklist || [],
            workspaceId: task.workspaceId || selectedWorkspace?.id || '',
      ...
      const updatedDestTasks = destTasks.map(
        (task, index) =>
          ({
            ...task,
            position: index,
  ...
  const handleTaskUpdated = (taskId: string, updates: Partial<AdminWorkspaceTask>) => {
    if (!selectedWorkspace) return;
      ...
      tasks: (col.tasks || []).map(task => {
        if (task.id === taskId) {
          ...
          return {
            ...
            assignee:
              cleanUpdates.assignee !== undefined ? cleanUpdates.assignee : task.assignee || '',
            assignees:
              cleanUpdates.assignees !== undefined ? cleanUpdates.assignees : task.assignees || [],
            assignedTo:
            ...
            labels: cleanUpdates.labels !== undefined ? cleanUpdates.labels : task.labels || [],
            checklist:
              cleanUpdates.checklist !== undefined ? cleanUpdates.checklist : task.checklist || [],
            workspaceId: task.workspaceId || selectedWorkspace?.id || '',
  ...
  const handleArchiveTask = async (taskId: string) => {
    ...
    try {
      ...
      let taskToArchive: AdminWorkspaceTask | null = null;
      ...
      const cleanTask = (
        ...
        return cleaned as AdminWorkspaceTask;
      };
      ...
      // Update Workspace using adminWorkspaceService
      await adminWorkspaceService.updateWorkspace(selectedWorkspace.id, {
        boardColumns: updatedColumns,
  ...
  const handleRestoreTask = async (taskId: string) => {
    ...
    try {
      ...
      const cleanTask = (task: AdminWorkspaceTask): AdminWorkspaceTask => {
        return {
          ...task,
          // Ensure all required AdminWorkspaceTask fields are present
          assignee: task.assignee || '',
      ...
      const cleanColumns = (columns: AdminWorkspaceBoardColumn[]): AdminWorkspaceBoardColumn[] => {
      ...
      // Update using adminWorkspaceService
      await adminWorkspaceService.updateWorkspace(selectedWorkspace.id, {
        boardColumns: updatedColumns,
