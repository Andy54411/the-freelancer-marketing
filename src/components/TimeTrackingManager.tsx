...
export default function TimeTrackingManager({
    ...
    const handleSubmitTimeEntry = async (e: React.FormEvent) => {
        ...
        try {
            ...
            if (editingEntry && editingEntry.id) {
                await TimeTracker.updateTimeEntry(editingEntry.id, entryData);
            } else {
                // Hole Customer ID aus Auftrag (vereinfacht)
                const customerId = 'customer_id'; // TODO: Real customer ID from order

                await TimeTracker.logTimeEntry(orderId, user.uid, customerId, entryData);
            }
    ...
    const handleDeleteEntry = async (entryId: string) => {
        ...
        try {
            await TimeTracker.deleteTimeEntry(entryId);
            await loadTimeTracking();
        } catch (error) {
    ...
    const handleSubmitForApproval = async () => {
        ...
        try {
            ...
            await TimeTracker.submitForCustomerApproval(
                orderId,
                user.uid,
                entryIds,
                message || undefined
                ...
                {timeEntries.length === 0 ? (
                    ...
                    <div className="space-y-3">
                        {timeEntries.map(entry => (
                            <div
                                key={entry.id}
                                className={`p-4 rounded-lg border-2 transition-all ${entry.status === 'customer_approved'
                                ...
                                <div className="flex items-start justify-between">
                                    <div className="flex-1">
                                        <div className="flex items-center gap-2 mb-1">
                                            ...
                                            <span
                                                className={`px-2 py-1 rounded-full text-xs font-medium ${entry.category === 'additional'
                                            ...
                                            <span
                                                className={`px-2 py-1 rounded-full text-xs font-medium ${entry.status === 'customer_approved'
