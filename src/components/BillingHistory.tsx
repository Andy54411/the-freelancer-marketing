...
export default function BillingHistory({ orderId, customerId, providerId }: BillingHistoryProps) {
  ...
  const loadBillingHistory = async () => {
    ...
    try {
      ...
      // For now, we'll create a mock billing history
      // In real implementation, this would fetch from Firebase
      const mockEntries: BillingEntry[] = [
        {
          id: '1',
          orderId: 'order_123',
          orderTitle: 'Badezimmer Renovation',
          providerName: 'Max Mustermann GmbH',
          ...
          timeEntry: {
            ...
            endTime: '15:00',
            hours: 6,
            description: 'Zusätzliche Fliesenarbeiten',
            category: 'additional',
            status: 'billed',
            createdAt: new Date() as any,
            billableAmount: 35000, // 350 EUR in cents
          },
          approvalRequest: {
            id: 'approval_1',
            orderId: 'order_123',
                ...
                <div className="flex-1">
                  <div className="flex items-center space-x-3 mb-2">
                    ...
                    <span className={`px-2 py-1 text-xs font-medium rounded-full ${getStatusColor(entry.status)}`}>
                      {entry.status === 'billed' ? 'Abgerechnet' : 
                       entry.status === 'paid' ? 'Bezahlt' : 'Streitig'}
                    </span>
                  </div>
