...
export const ExecutivePremiumDeliveryTemplate: React.FC<TemplateProps> = ({
  ...
  return (
    <div className="max-w-4xl mx-auto bg-white p-10 font-sans text-gray-900">
      ...
      {/* Titel */}
      <div className="flex items-center justify-between mb-6 pb-2 border-b border-green-300">
        <div className="text-3xl tracking-wide font-light text-green-800">Lieferschein</div>
      </div>
      ...
      {/* Tabelle mit grüner Farbe */}
      {data.items && data.items.length > 0 && (
        <div className="mb-10">
          <table className="w-full border-collapse">
            <thead>
              <tr className="bg-green-600 text-white">
                <th className="p-3 text-left text-xs font-semibold tracking-wide">Pos.</th>
                <th className="p-3 text-left text-xs font-semibold tracking-wide">Art-Nr.</th>
                <th className="p-3 text-left text-xs font-semibold tracking-wide">Bezeichnung</th>
                <th className="p-3 text-center text-xs font-semibold tracking-wide">Menge</th>
                <th className="p-3 text-center text-xs font-semibold tracking-wide">Einheit</th>
              </tr>
            </thead>
            <tbody>
              {data.items.map((item, idx) => {
                ...
                return (
                  <tr key={idx} className="border-b border-gray-200 align-top hover:bg-green-50">
                    <td className="p-3 text-gray-700">{idx + 1}</td>
                    <td className="p-3 text-gray-700 whitespace-nowrap">{artNr || '—'}</td>
                    <td className="p-3">
                    ...
                    <td className="p-3 text-center text-gray-700">{item.unit || 'Stk.'}</td>
                  </tr>
                );
              })}
            </tbody>
      ...
      {/* Footer mit grünem Akzent */}
      <div className="mt-10 pt-4 border-t border-green-300 text-xs text-gray-600">
        <div className="text-center">
          <div className="text-green-700 font-medium">Executive Premium • Exzellenter Service</div>
          <div className="mt-2">
...
export default ExecutivePremiumDeliveryTemplate;
