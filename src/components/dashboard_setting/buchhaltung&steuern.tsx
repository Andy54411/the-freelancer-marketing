...
const AccountingForm: React.FC<AccountingFormProps> = ({ formData, handleChange }) => {
  ...
  const getBoxClass = (selected: boolean, disabled = false) => {
    if (disabled) {
      return (
        baseBoxClass + 'opacity-50 cursor-not-allowed border-gray-600 bg-gray-700 text-gray-400'
      );
    }
    if (selected) {
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        <div>
          ...
          <p className={helperTextClass}>
            Diese Angaben benötigst du auf deinen Dokumenten, wenn du im Handelsregister eingetragen
            bist.
          </p>
        </div>
        <div>
          ...
          {['skro4', 'skro3'].map(type => (
            ...
            >
              ...
              <p className="text-sm text-gray-700 dark:text-gray-300">
                {type === 'skro4'
                  ? 'angelehnt an Jahresabschluss'
                  : 'angelehnt an Unternehmensabläufe'}
              </p>
            </div>
          ))}
        </div>
