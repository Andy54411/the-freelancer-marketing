...
const LogoForm: React.FC<LogoFormProps> = ({ formData, handleChange }) => {
  ...
  console.log(
    'DEBUG LogoForm (initial): formData?.step3?.profilePictureURL:',
  ...
  useEffect(() => {
    ...
    const fetchProjectImagesFromFirestore = async () => {
      ...
      if (userDocSnap.exists()) {
        ...
        console.log(
          'DEBUG LogoForm (useEffect): projectImages from Firestore userData:',
          userData.projectImages
        );
  ...
  const handleProjectUpload = async (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      console.log(
        'DEBUG LogoForm (handleProjectUpload): All Project Image URLs for Firestore:',
  ...
  const handleDelete = async (url: string) => {
    ...
    try {
      ...
      console.log(
        'DEBUG LogoForm (handleDelete): Updated Project Image URLs:',
        updatedProjectImages
      );
      await updateDoc(doc(db, 'users', uid), {
  ...
  return (
    <div className="bg-white dark:bg-gray-800 rounded-lg shadow-sm p-6 space-y-6">
      ...
      <div>
        ...
        <label
          htmlFor="project-images-upload"
          className="cursor-pointer inline-flex items-center gap-2 bg-[#14ad9f] text-white px-4 py-2 rounded hover:bg-teal-700"
        >
          <FiUpload />
          Upload
          <input
