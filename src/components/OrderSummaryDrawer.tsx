...
export function OrderSummaryDrawer({ order, isOpen, onOpenChange, providerName }: OrderSummaryDrawerProps) {
    ...
    const handleSendQuickMessage = async () => {
        ...
        try {
            ...
            await setDoc(chatDocRef, { users: [order.orderedBy, order.uid], lastMessage: lastMessagePayload, lastUpdated: serverTimestamp() }, { merge: true });
    ...
    const formattedPrice = new Intl.NumberFormat("de-DE", { style: "currency", currency: "EUR" }).format(order.totalAmountPaidByBuyer / 100);

    const formattedDate = order.orderDate ? new Date(
        ...
        <Drawer open={isOpen} onOpenChange={onOpenChange} direction="right">
            <DrawerContent className="h-full w-full max-w-md ml-auto bg-background">
                    ...
                    {['AKTIV', 'IN BEARBEITUNG'].includes(order.status) && (
                        <>
                            ...
                            <div className="pt-6 space-y-3">
                                <h3 className="text-base font-semibold text-gray-800 dark:text-gray-100">Schnellnachricht senden</h3>
                                <Textarea
                                ...
                                />
                                <Button onClick={handleSendQuickMessage} disabled={isSending || !quickMessage.trim()} className="w-full bg-[#14ad9f] text-white hover:bg-teal-700">
                ...
                <DrawerFooter className="mt-auto p-6 border-t bg-background">
                    <Button asChild className="bg-[#14ad9f] text-white hover:bg-teal-700"><Link href={`/dashboard/company/${order.uid}/orders/${order.id}`}>Vollständige Auftragsdetails</Link></Button>
                    <DrawerClose asChild><Button variant="outline">Schließen</Button></DrawerClose>
                </DrawerFooter>
            </DrawerContent>
        </Drawer>
    );
