...
export const invoicePlaceholders: PlaceholderRegistry = {
  ...
  'LEISTUNGSDATUM': (context: PlaceholderContext) => {
    ...
    const date = invoice.serviceDate.toDate ? invoice.serviceDate.toDate() : new Date(invoice.serviceDate);
    return date.toLocaleDateString('de-DE');
  },
  ...
  'LEISTUNGSZEITRAUM': (context: PlaceholderContext) => {
    ...
    const startDate = invoice.servicePeriodStart.toDate ? 
      invoice.servicePeriodStart.toDate() : new Date(invoice.servicePeriodStart);
    const endDate = invoice.servicePeriodEnd.toDate ? 
      invoice.servicePeriodEnd.toDate() : new Date(invoice.servicePeriodEnd);
  ...
  'WAEHRUNG': (context: PlaceholderContext) => 
  ...
  'ZAHLUNGSART': (context: PlaceholderContext) => 
  ...
  'BESTELLDATUM': (context: PlaceholderContext) => {
    ...
    const date = invoice.orderDate.toDate ? invoice.orderDate.toDate() : new Date(invoice.orderDate);
    return date.toLocaleDateString('de-DE');
  },
  
  'PROJEKTNUMMER': (context: PlaceholderContext) => 
    context.invoice?.projectNumber || '',
    
  'PROJEKTNAME': (context: PlaceholderContext) => 
    context.invoice?.projectName || '',
  ...
  'LIEFERADRESSE': (context: PlaceholderContext) => 
    context.invoice?.deliveryAddress || '',
    
  'LIEFERDATUM': (context: PlaceholderContext) => {
    const invoice = context.invoice;
    if (!invoice?.deliveryDate) return '';
    
    const date = invoice.deliveryDate.toDate ? invoice.deliveryDate.toDate() : new Date(invoice.deliveryDate);
    return date.toLocaleDateString('de-DE');
  },
  ...
  'INTERNE_NOTIZEN': (context: PlaceholderContext) => 
    context.invoice?.internalNotes || '',
    
  'ZAHLUNGSHINWEIS': (context: PlaceholderContext) => 
    context.invoice?.paymentNote || '',
  
  // Storno-spezifische Platzhalter
  'STORNO_GRUND': (context: PlaceholderContext) => 
    context.invoice?.cancelReason || '',
    
  'STORNO_DATUM': (context: PlaceholderContext) => {
    const invoice = context.invoice;
    if (!invoice?.cancelDate) return '';
    
    const date = invoice.cancelDate.toDate ? invoice.cancelDate.toDate() : new Date(invoice.cancelDate);
    return date.toLocaleDateString('de-DE');
  },
