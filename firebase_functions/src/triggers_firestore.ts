...
// Imports für V1-Funktionen
import * as functions from 'firebase-functions'; // <- WICHTIG: Verwende jetzt functions aus V1
// import { onDocumentCreated, onDocumentUpdated } from 'firebase-functions/v2/firestore'; // <- V2-Imports entfernt
import { logger as loggerV2 } from 'firebase-functions/v2'; // <- V2-Logger beibehalten
// import * as functionsV1 from 'firebase-functions/v1'; // <- Nicht mehr benötigt, da functions aus V1 importiert wird
...
export const createUserProfile = functions.firestore // <- functions.firestore statt onDocumentCreated
  .document("users/{userId}")
  .onCreate(async (snapshot, context) => { // <- snapshot und context direkt, kein event
...
export const updateUserProfile = functions.firestore // <- functions.firestore statt onDocumentUpdated
  .document("users/{userId}")
  .onUpdate(async (change, context) => { // <- change und context direkt, kein event
    const userId = context.params.userId;
    const snapshotAfter = change.after; // <- change.after statt event.data.after
    if (!snapshotAfter) {
      loggerV2.warn(`[updateUserProfile] Kein Daten-Snapshot nach Update für User ${userId}. Abbruch.`);
      return null;
    }
    const userData = snapshotAfter.data() as FirmaUserData; // <- .data()
...
export const createStripeCustomAccountOnUserUpdate = functions.firestore // <- functions.firestore statt functionsV1.firestore
  .document("users/{userId}")
  .onUpdate(async (change, context) => {
    const userId = context.params.userId;
    loggerV2.info(`Firestore Trigger 'createStripeCustomAccountOnUserUpdate' (V1) für ${userId}.`); // <- loggerV2
    const localStripe = getStripeInstance();
    const after = change.after.data() as FirmaUserData; // <- .data()
    if (!after) {
